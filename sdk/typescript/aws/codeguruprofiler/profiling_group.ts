// Code generated by `ubx sdk gen` from aws_code_guru_profiler_profiling_group. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface ProfilingGroup_AgentPermissions {
  /** The principals for the agent permissions. */
  principals: string[] | Computed<string[]>;
}

export interface ProfilingGroup_AnomalyDetectionNotificationConfiguration {
  /** The unique identifier assigned by CodeGuru Profiler to an Amazon SNS notification channel used for anomaly detection, which references the channel within the profiling group's configuration. (AI-inferred) */
  channelId?: string | Computed<string>;
  /** The ARN of the SNS topic to which anomaly detection notifications are published for this profiling group. (AI-inferred) */
  channelUri?: string | Computed<string>;
}

export interface ProfilingGroup_Tags {
  /** A key in the tags map of an AWS CodeGuru Profiler profiling group, used to attach custom metadata (such as environment or application name) to the profiling group for resource organization, cost allocation, and access control. (AI-inferred) */
  key?: string | Computed<string>;
  /** The value of a user-defined tag assigned to the profiling group, used for cost allocation, resource management, and access control in AWS. (AI-inferred) */
  value?: string | Computed<string>;
}

const ProfilingGroup_AgentPermissionsFields: FieldMap = {
  principals: "principals",
};

const ProfilingGroup_AnomalyDetectionNotificationConfigurationFields: FieldMap = {
  channelId: "channel_id",
  channelUri: "channel_uri",
};

const ProfilingGroup_TagsFields: FieldMap = {
  key: "key",
  value: "value",
};

export interface ProfilingGroupConfig {
  /** The agent permissions attached to this profiling group. */
  agentPermissions?: ProfilingGroup_AgentPermissions | Computed<ProfilingGroup_AgentPermissions>;
  /** Configuration for Notification Channels for Anomaly Detection feature in CodeGuru Profiler which enables customers to detect anomalies in the application profile for those methods that represent the highest proportion of CPU time or latency */
  anomalyDetectionNotificationConfiguration?: ProfilingGroup_AnomalyDetectionNotificationConfiguration[] | Computed<ProfilingGroup_AnomalyDetectionNotificationConfiguration[]>;
  /** The compute platform of the profiling group. */
  computePlatform?: string | Computed<string>;
  /** The name of the profiling group. */
  profilingGroupName: string | Computed<string>;
  /** The tags associated with a profiling group. */
  tags?: ProfilingGroup_Tags[] | Computed<ProfilingGroup_Tags[]>;
}

export interface ProfilingGroupAttrs {
  /** The agent permissions attached to this profiling group. */
  agentPermissions: ProfilingGroup_AgentPermissions;
  /** Configuration for Notification Channels for Anomaly Detection feature in CodeGuru Profiler which enables customers to detect anomalies in the application profile for those methods that represent the highest proportion of CPU time or latency */
  anomalyDetectionNotificationConfiguration: ProfilingGroup_AnomalyDetectionNotificationConfiguration[];
  /** The Amazon Resource Name (ARN) that uniquely identifies the profiling group. (AI-inferred) */
  arn: string;
  /** The compute platform of the profiling group. */
  computePlatform: string;
  /** The name of the profiling group. */
  profilingGroupName: string;
  /** The tags associated with a profiling group. */
  tags: ProfilingGroup_Tags[];
}

export const ProfilingGroup: ResourceBinding<ProfilingGroupConfig, ProfilingGroupAttrs> = {
  wireType: "aws_code_guru_profiler_profiling_group",
  fields: {
    agentPermissions: {
      wireName: "agent_permissions",
      kind: "object",
      fields: ProfilingGroup_AgentPermissionsFields,
    },
    anomalyDetectionNotificationConfiguration: {
      wireName: "anomaly_detection_notification_configuration",
      kind: "list",
      fields: ProfilingGroup_AnomalyDetectionNotificationConfigurationFields,
    },
    computePlatform: "compute_platform",
    profilingGroupName: "profiling_group_name",
    tags: {
      wireName: "tags",
      kind: "list",
      fields: ProfilingGroup_TagsFields,
    },
  },
};
