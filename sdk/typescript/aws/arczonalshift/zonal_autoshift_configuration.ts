// Code generated by `ubx sdk gen` from aws_arczonal_shift_zonal_autoshift_configuration. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface ZonalAutoshiftConfiguration_PracticeRunConfiguration_BlockingAlarms {
  /** The identifier (ARN or name) of a CloudWatch alarm that, when in ALARM state, blocks or stops zonal autoshift practice runs. (AI-inferred) */
  alarmIdentifier?: string | Computed<string>;
  /** Specifies whether the CloudWatch alarm in the blocking alarms list is a 'BLOCKING' type (as opposed to 'OUTCOME'), which is required for blocking alarms to prevent zonal autoshift when the alarm enters ALARM state. (AI-inferred) */
  type?: string | Computed<string>;
}

export interface ZonalAutoshiftConfiguration_PracticeRunConfiguration {
  /** A list of dates in YYYY-MM-DD format on which zonal autoshift practice runs are blocked and cannot occur. (AI-inferred) */
  blockedDates?: string[] | Computed<string[]>;
  /** A list of cron expressions in UTC that define recurring time windows during which the service will not start a practice run for this zonal autoshift configuration. (AI-inferred) */
  blockedWindows?: string[] | Computed<string[]>;
  /** Specifies the list of CloudWatch alarms that, when in ALARM state, block the practice run from starting (or continuing) for the zonal autoshift configuration. (AI-inferred) */
  blockingAlarms?: ZonalAutoshiftConfiguration_PracticeRunConfiguration_BlockingAlarms[] | Computed<ZonalAutoshiftConfiguration_PracticeRunConfiguration_BlockingAlarms[]>;
  /** Specifies the CloudWatch alarms used to evaluate the outcome of zonal autoshift practice runs, where an alarm entering the ALARM state during a practice run marks it as failed. (AI-inferred) */
  outcomeAlarms: ZonalAutoshiftConfiguration_PracticeRunConfiguration_BlockingAlarms[] | Computed<ZonalAutoshiftConfiguration_PracticeRunConfiguration_BlockingAlarms[]>;
}

const ZonalAutoshiftConfiguration_PracticeRunConfiguration_BlockingAlarmsFields: FieldMap = {
  alarmIdentifier: "alarm_identifier",
  type: "type",
};

const ZonalAutoshiftConfiguration_PracticeRunConfigurationFields: FieldMap = {
  blockedDates: "blocked_dates",
  blockedWindows: "blocked_windows",
  blockingAlarms: {
    wireName: "blocking_alarms",
    kind: "list",
    fields: ZonalAutoshiftConfiguration_PracticeRunConfiguration_BlockingAlarmsFields,
  },
  outcomeAlarms: {
    wireName: "outcome_alarms",
    kind: "list",
    fields: ZonalAutoshiftConfiguration_PracticeRunConfiguration_BlockingAlarmsFields,
  },
};

export interface ZonalAutoshiftConfigurationConfig {
  /** Defines the practice run configuration for zonal autoshift, specifying blocking windows/dates and the outcome alarms that must clear for a practice run to be considered successful. (AI-inferred) */
  practiceRunConfiguration?: ZonalAutoshiftConfiguration_PracticeRunConfiguration | Computed<ZonalAutoshiftConfiguration_PracticeRunConfiguration>;
  /** The identifier (such as an ARN) of the resource for which the zonal autoshift configuration is set, enabling automatic traffic shifting away from an Availability Zone for that resource. (AI-inferred) */
  resourceIdentifier?: string | Computed<string>;
  /** Sets the desired zonal autoshift status for the configuration, with allowed values 'ENABLED' or 'DISABLED'. (AI-inferred) */
  zonalAutoshiftStatus?: string | Computed<string>;
}

export interface ZonalAutoshiftConfigurationAttrs {
  /** Defines the practice run configuration for zonal autoshift, specifying blocking windows/dates and the outcome alarms that must clear for a practice run to be considered successful. (AI-inferred) */
  practiceRunConfiguration: ZonalAutoshiftConfiguration_PracticeRunConfiguration;
  /** The identifier (such as an ARN) of the resource for which the zonal autoshift configuration is set, enabling automatic traffic shifting away from an Availability Zone for that resource. (AI-inferred) */
  resourceIdentifier: string;
  /** Sets the desired zonal autoshift status for the configuration, with allowed values 'ENABLED' or 'DISABLED'. (AI-inferred) */
  zonalAutoshiftStatus: string;
}

export const ZonalAutoshiftConfiguration: ResourceBinding<ZonalAutoshiftConfigurationConfig, ZonalAutoshiftConfigurationAttrs> = {
  wireType: "aws_arczonal_shift_zonal_autoshift_configuration",
  fields: {
    practiceRunConfiguration: {
      wireName: "practice_run_configuration",
      kind: "object",
      fields: ZonalAutoshiftConfiguration_PracticeRunConfigurationFields,
    },
    resourceIdentifier: "resource_identifier",
    zonalAutoshiftStatus: "zonal_autoshift_status",
  },
};
