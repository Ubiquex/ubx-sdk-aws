// Code generated by `ubx sdk gen` from aws_data_zone_policy_grant. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface PolicyGrant_Detail_AddToProjectMemberPool {
  /** Whether the policy grant also applies to child domain units when adding a member to the project member pool. (AI-inferred) */
  includeChildDomainUnits?: boolean | Computed<boolean>;
}

export interface PolicyGrant_Detail_CreateEnvironmentProfile {
  /** The identifier of the DataZone domain unit to which the create environment profile policy grant applies. (AI-inferred) */
  domainUnitId?: string | Computed<string>;
}

export interface PolicyGrant_Detail_CreateProjectFromProjectProfile {
  /** Whether the policy grant for creating a project from a project profile also applies to child domain units, enabling the create-project action to be performed in those child domain units as well. (AI-inferred) */
  includeChildDomainUnits?: boolean | Computed<boolean>;
  /** The list of DataZone project profile identifiers that the policy grant applies to, allowing the grantee to create projects from these specific project profiles. (AI-inferred) */
  projectProfiles?: string[] | Computed<string[]>;
}

export interface PolicyGrant_Detail {
  /** Configures the ADD_TO_PROJECT_MEMBER_POOL policy grant, determining the settings that allow the grantee to add members to a project's member pool. (AI-inferred) */
  addToProjectMemberPool?: PolicyGrant_Detail_AddToProjectMemberPool | Computed<PolicyGrant_Detail_AddToProjectMemberPool>;
  /** For an AWS DataZone policy grant, this nested object configures the CreateAssetType policy, where its 'include_child_domain_units' field determines whether the permission to create asset types also applies to child domain units. (AI-inferred) */
  createAssetType?: PolicyGrant_Detail_AddToProjectMemberPool | Computed<PolicyGrant_Detail_AddToProjectMemberPool>;
  /** Configures the 'Create Domain Unit' policy grant detail, an empty object that, when present, grants the principal permission to create domain units within the DataZone domain. (AI-inferred) */
  createDomainUnit?: PolicyGrant_Detail_AddToProjectMemberPool | Computed<PolicyGrant_Detail_AddToProjectMemberPool>;
  createEnvironment?: unknown | Computed<unknown>;
  /** Specifies the policy grant detail that grants the principal permission to create an environment from a blueprint in Amazon Data Zone. (AI-inferred) */
  createEnvironmentFromBlueprint?: unknown | Computed<unknown>;
  /** This object contains the policy grant details for allowing a principal to create environment profiles in AWS DataZone, including the domain units under which the permission applies. (AI-inferred) */
  createEnvironmentProfile?: PolicyGrant_Detail_CreateEnvironmentProfile | Computed<PolicyGrant_Detail_CreateEnvironmentProfile>;
  /** Specifies the policy grant details for creating form types, including whether the permission also applies to child domain units. (AI-inferred) */
  createFormType?: PolicyGrant_Detail_AddToProjectMemberPool | Computed<PolicyGrant_Detail_AddToProjectMemberPool>;
  /** This object defines the policy grant details for creating glossaries in Amazon DataZone, allowing the specified principal to create glossary entries within the target domain or project. (AI-inferred) */
  createGlossary?: PolicyGrant_Detail_AddToProjectMemberPool | Computed<PolicyGrant_Detail_AddToProjectMemberPool>;
  /** The create_project details of the policy grant, used when the policy type is set to CREATE_PROJECT, specifying the permissions for creating projects in Amazon Data Zone. (AI-inferred) */
  createProject?: PolicyGrant_Detail_AddToProjectMemberPool | Computed<PolicyGrant_Detail_AddToProjectMemberPool>;
  /** The create_project_from_project_profile object defines the policy grant configuration for allowing a principal to create projects from a project profile, typically containing the list of project profile identifiers the grant applies to. (AI-inferred) */
  createProjectFromProjectProfile?: PolicyGrant_Detail_CreateProjectFromProjectProfile | Computed<PolicyGrant_Detail_CreateProjectFromProjectProfile>;
  /** When enabled, this flag allows the policy grantee to delegate the ability to create environment profiles to other principals. (AI-inferred) */
  delegateCreateEnvironmentProfile?: unknown | Computed<unknown>;
  /** This nested object represents the details of an 'override domain unit owners' policy grant, with an `enabled` boolean indicating whether the grant overrides the domain unit's existing owner list. (AI-inferred) */
  overrideDomainUnitOwners?: PolicyGrant_Detail_AddToProjectMemberPool | Computed<PolicyGrant_Detail_AddToProjectMemberPool>;
  /** This object contains the detail settings for a policy grant of type OVERRIDE_PROJECT_OWNERS, which grants the principal the ability to override project ownership within AWS DataZone. (AI-inferred) */
  overrideProjectOwners?: PolicyGrant_Detail_AddToProjectMemberPool | Computed<PolicyGrant_Detail_AddToProjectMemberPool>;
}

export interface PolicyGrant_Principal_DomainUnit_DomainUnitGrantFilter {
  /** When set to an empty object, this field configures the policy grant to apply to all domain units within the DataZone domain. (AI-inferred) */
  allDomainUnitsGrantFilter?: unknown | Computed<unknown>;
}

export interface PolicyGrant_Principal_DomainUnit {
  /** Specifies the role (owner or member) within the domain unit principal that the policy grant applies to, controlling which users in that domain unit are granted the policy. (AI-inferred) */
  domainUnitDesignation?: string | Computed<string>;
  /** Defines a filter that specifies whether the policy grant applies to all domain units within the domain unit principal or only to a specific list of domain unit IDs, used to scope the grant for a domain unit principal. (AI-inferred) */
  domainUnitGrantFilter?: PolicyGrant_Principal_DomainUnit_DomainUnitGrantFilter | Computed<PolicyGrant_Principal_DomainUnit_DomainUnitGrantFilter>;
  /** The identifier of the Amazon DataZone domain unit that acts as the principal for this policy grant, determining which domain unit receives the granted permissions. (AI-inferred) */
  domainUnitIdentifier?: string | Computed<string>;
}

export interface PolicyGrant_Principal_Group {
  /** The identifier of the group that is the principal receiving the policy grant in AWS DataZone. (AI-inferred) */
  groupIdentifier?: string | Computed<string>;
}

export interface PolicyGrant_Principal_Project_ProjectGrantFilter_DomainUnitFilter {
  /** The identifier of the AWS Data Zone domain unit used by the domain unit filter to scope this policy grant for the specified project principal. (AI-inferred) */
  domainUnit?: string | Computed<string>;
  /** Specifies whether the policy grant to the project principal also applies to all child domain units underneath the domain unit selected in this filter. (AI-inferred) */
  includeChildDomainUnits?: boolean | Computed<boolean>;
}

export interface PolicyGrant_Principal_Project_ProjectGrantFilter {
  /** Specifies a domain unit filter that scopes the project-level policy grant to apply only to the designated domain units within the data zone. (AI-inferred) */
  domainUnitFilter?: PolicyGrant_Principal_Project_ProjectGrantFilter_DomainUnitFilter | Computed<PolicyGrant_Principal_Project_ProjectGrantFilter_DomainUnitFilter>;
}

export interface PolicyGrant_Principal_Project {
  /** Specifies whether the project principal is identified by its project ID or its project name, enabling the policy grant to reference a project as the grant principal. (AI-inferred) */
  projectDesignation?: string | Computed<string>;
  /** Configures a domain unit filter that restricts the policy grant to only members of the project who belong to the specified domain unit. (AI-inferred) */
  projectGrantFilter?: PolicyGrant_Principal_Project_ProjectGrantFilter | Computed<PolicyGrant_Principal_Project_ProjectGrantFilter>;
  /** The identifier of the Amazon DataZone project that serves as the principal for the policy grant, granting permissions to that project. (AI-inferred) */
  projectIdentifier?: string | Computed<string>;
}

export interface PolicyGrant_Principal_User {
  /** A boolean flag that, when set to true, grants the policy to all users in the DataZone domain rather than to a specific user identifier. (AI-inferred) */
  allUsersGrantFilter?: unknown | Computed<unknown>;
  /** Specifies the user identifier of the AWS DataZone user who is the principal receiving this policy grant, determining which user the granted permissions apply to. (AI-inferred) */
  userIdentifier?: string | Computed<string>;
}

export interface PolicyGrant_Principal {
  /** Identifies a domain unit as the principal for the policy grant, so that the grant applies to all users and groups within that domain unit. (AI-inferred) */
  domainUnit?: PolicyGrant_Principal_DomainUnit | Computed<PolicyGrant_Principal_DomainUnit>;
  /** Specifies the group principal for the policy grant, containing the unique identifier of the AWS DataZone group that receives the granted permissions. (AI-inferred) */
  group?: PolicyGrant_Principal_Group | Computed<PolicyGrant_Principal_Group>;
  /** The project object representing the Amazon Data Zone project that will receive the permissions granted by this policy grant. (AI-inferred) */
  project?: PolicyGrant_Principal_Project | Computed<PolicyGrant_Principal_Project>;
  /** Defines the user principal to which the policy grant is assigned, specifying the Amazon DataZone user by its unique identifier. (AI-inferred) */
  user?: PolicyGrant_Principal_User | Computed<PolicyGrant_Principal_User>;
}

const PolicyGrant_Detail_AddToProjectMemberPoolFields: FieldMap = {
  includeChildDomainUnits: "include_child_domain_units",
};

const PolicyGrant_Detail_CreateEnvironmentProfileFields: FieldMap = {
  domainUnitId: "domain_unit_id",
};

const PolicyGrant_Detail_CreateProjectFromProjectProfileFields: FieldMap = {
  includeChildDomainUnits: "include_child_domain_units",
  projectProfiles: "project_profiles",
};

const PolicyGrant_DetailFields: FieldMap = {
  addToProjectMemberPool: {
    wireName: "add_to_project_member_pool",
    kind: "object",
    fields: PolicyGrant_Detail_AddToProjectMemberPoolFields,
  },
  createAssetType: {
    wireName: "create_asset_type",
    kind: "object",
    fields: PolicyGrant_Detail_AddToProjectMemberPoolFields,
  },
  createDomainUnit: {
    wireName: "create_domain_unit",
    kind: "object",
    fields: PolicyGrant_Detail_AddToProjectMemberPoolFields,
  },
  createEnvironment: "create_environment",
  createEnvironmentFromBlueprint: "create_environment_from_blueprint",
  createEnvironmentProfile: {
    wireName: "create_environment_profile",
    kind: "object",
    fields: PolicyGrant_Detail_CreateEnvironmentProfileFields,
  },
  createFormType: {
    wireName: "create_form_type",
    kind: "object",
    fields: PolicyGrant_Detail_AddToProjectMemberPoolFields,
  },
  createGlossary: {
    wireName: "create_glossary",
    kind: "object",
    fields: PolicyGrant_Detail_AddToProjectMemberPoolFields,
  },
  createProject: {
    wireName: "create_project",
    kind: "object",
    fields: PolicyGrant_Detail_AddToProjectMemberPoolFields,
  },
  createProjectFromProjectProfile: {
    wireName: "create_project_from_project_profile",
    kind: "object",
    fields: PolicyGrant_Detail_CreateProjectFromProjectProfileFields,
  },
  delegateCreateEnvironmentProfile: "delegate_create_environment_profile",
  overrideDomainUnitOwners: {
    wireName: "override_domain_unit_owners",
    kind: "object",
    fields: PolicyGrant_Detail_AddToProjectMemberPoolFields,
  },
  overrideProjectOwners: {
    wireName: "override_project_owners",
    kind: "object",
    fields: PolicyGrant_Detail_AddToProjectMemberPoolFields,
  },
};

const PolicyGrant_Principal_DomainUnit_DomainUnitGrantFilterFields: FieldMap = {
  allDomainUnitsGrantFilter: "all_domain_units_grant_filter",
};

const PolicyGrant_Principal_DomainUnitFields: FieldMap = {
  domainUnitDesignation: "domain_unit_designation",
  domainUnitGrantFilter: {
    wireName: "domain_unit_grant_filter",
    kind: "object",
    fields: PolicyGrant_Principal_DomainUnit_DomainUnitGrantFilterFields,
  },
  domainUnitIdentifier: "domain_unit_identifier",
};

const PolicyGrant_Principal_GroupFields: FieldMap = {
  groupIdentifier: "group_identifier",
};

const PolicyGrant_Principal_Project_ProjectGrantFilter_DomainUnitFilterFields: FieldMap = {
  domainUnit: "domain_unit",
  includeChildDomainUnits: "include_child_domain_units",
};

const PolicyGrant_Principal_Project_ProjectGrantFilterFields: FieldMap = {
  domainUnitFilter: {
    wireName: "domain_unit_filter",
    kind: "object",
    fields: PolicyGrant_Principal_Project_ProjectGrantFilter_DomainUnitFilterFields,
  },
};

const PolicyGrant_Principal_ProjectFields: FieldMap = {
  projectDesignation: "project_designation",
  projectGrantFilter: {
    wireName: "project_grant_filter",
    kind: "object",
    fields: PolicyGrant_Principal_Project_ProjectGrantFilterFields,
  },
  projectIdentifier: "project_identifier",
};

const PolicyGrant_Principal_UserFields: FieldMap = {
  allUsersGrantFilter: "all_users_grant_filter",
  userIdentifier: "user_identifier",
};

const PolicyGrant_PrincipalFields: FieldMap = {
  domainUnit: {
    wireName: "domain_unit",
    kind: "object",
    fields: PolicyGrant_Principal_DomainUnitFields,
  },
  group: {
    wireName: "group",
    kind: "object",
    fields: PolicyGrant_Principal_GroupFields,
  },
  project: {
    wireName: "project",
    kind: "object",
    fields: PolicyGrant_Principal_ProjectFields,
  },
  user: {
    wireName: "user",
    kind: "object",
    fields: PolicyGrant_Principal_UserFields,
  },
};

export interface PolicyGrantConfig {
  detail?: PolicyGrant_Detail | Computed<PolicyGrant_Detail>;
  /** The identifier of the Amazon DataZone domain where the policy grant is created. (AI-inferred) */
  domainIdentifier: string | Computed<string>;
  /** The identifier of the Data Zone entity (such as a project, domain, or user) to which the policy grant applies. (AI-inferred) */
  entityIdentifier: string | Computed<string>;
  /** Specifies the type of DataZone entity (such as 'project', 'user', or 'group') that this policy grant targets, used together with the entity identifier to define the grant's scope. (AI-inferred) */
  entityType: string | Computed<string>;
  /** Specifies the type of policy grant being created, such as CREATE_DOMAIN_UNIT or DELETE_DOMAIN_UNIT, which dictates the specific permission granted on the target entity. (AI-inferred) */
  policyType: string | Computed<string>;
  /** The user, group, or role to which the policy grant is assigned, specifying the principal type and identifier to control which entity receives the granted permissions. (AI-inferred) */
  principal?: PolicyGrant_Principal | Computed<PolicyGrant_Principal>;
}

export interface PolicyGrantAttrs {
  /** Specifies the timestamp at which policy grant member was created. */
  createdAt: string;
  /** Specifies the user who created the policy grant member. */
  createdBy: string;
  detail: PolicyGrant_Detail;
  /** The identifier of the Amazon DataZone domain where the policy grant is created. (AI-inferred) */
  domainIdentifier: string;
  /** The identifier of the Data Zone entity (such as a project, domain, or user) to which the policy grant applies. (AI-inferred) */
  entityIdentifier: string;
  /** Specifies the type of DataZone entity (such as 'project', 'user', or 'group') that this policy grant targets, used together with the entity identifier to define the grant's scope. (AI-inferred) */
  entityType: string;
  /** The unique identifier of the policy grant returned by the AddPolicyGrant API */
  grantId: string;
  /** Specifies the type of policy grant being created, such as CREATE_DOMAIN_UNIT or DELETE_DOMAIN_UNIT, which dictates the specific permission granted on the target entity. (AI-inferred) */
  policyType: string;
  /** The user, group, or role to which the policy grant is assigned, specifying the principal type and identifier to control which entity receives the granted permissions. (AI-inferred) */
  principal: PolicyGrant_Principal;
}

export const PolicyGrant: ResourceBinding<PolicyGrantConfig, PolicyGrantAttrs> = {
  wireType: "aws_data_zone_policy_grant",
  fields: {
    detail: {
      wireName: "detail",
      kind: "object",
      fields: PolicyGrant_DetailFields,
    },
    domainIdentifier: "domain_identifier",
    entityIdentifier: "entity_identifier",
    entityType: "entity_type",
    policyType: "policy_type",
    principal: {
      wireName: "principal",
      kind: "object",
      fields: PolicyGrant_PrincipalFields,
    },
  },
};
