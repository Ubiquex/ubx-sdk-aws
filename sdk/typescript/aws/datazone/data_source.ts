// Code generated by `ubx sdk gen` from aws_data_zone_data_source. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface DataSource_AssetFormsInput {
  /** The JSON-encoded content of the asset form input, which provides the metadata values to be associated with the asset when the data source is run. (AI-inferred) */
  content?: string | Computed<string>;
  /** The name of the metadata form to associate with each asset ingested through this data source, determining which form schema is applied to the asset metadata. (AI-inferred) */
  formName?: string | Computed<string>;
  /** The identifier of the metadata form type that defines the structure and fields for this asset form input, used when creating or updating assets in Amazon DataZone. (AI-inferred) */
  typeIdentifier?: string | Computed<string>;
  /** The revision identifier of the asset form type to apply, indicating which version of the form schema the data source should use when creating an asset's metadata. (AI-inferred) */
  typeRevision?: string | Computed<string>;
}

export interface DataSource_Configuration_GlueRunConfiguration_RelationalFilterConfigurations_FilterExpressions {
  /** The SQL-like filter expression string used to include or exclude specific rows from the source table when the DataZone data source runs against the Glue relational configuration. (AI-inferred) */
  expression?: string | Computed<string>;
  /** The type of the filter expression, which indicates whether the expression is an inclusion (INCLUDE) or exclusion (EXCLUDE) rule for the relational data source tables. (AI-inferred) */
  type?: string | Computed<string>;
}

export interface DataSource_Configuration_GlueRunConfiguration_RelationalFilterConfigurations {
  /** The name of the AWS Glue database in the Data Catalog that the DataZone data source reads from when using this Glue run configuration. (AI-inferred) */
  databaseName?: string | Computed<string>;
  /** List of filter expressions, each containing an expression string and a type (e.g., INCLUDE or EXCLUDE), that determine which database objects (tables or views) in the corresponding relational data source are included in or excluded from metadata ingestion in AWS DataZone. (AI-inferred) */
  filterExpressions?: DataSource_Configuration_GlueRunConfiguration_RelationalFilterConfigurations_FilterExpressions[] | Computed<DataSource_Configuration_GlueRunConfiguration_RelationalFilterConfigurations_FilterExpressions[]>;
  /** For an AWS DataZone data source with a Glue run configuration, this schema name specifies the database schema within the relational database whose tables are considered for ingestion, as refined by the associated filter expressions. (AI-inferred) */
  schemaName?: string | Computed<string>;
}

export interface DataSource_Configuration_GlueRunConfiguration {
  /** When set to true, this boolean enables automatic import of AWS Glue Data Quality results into Amazon DataZone for the data source. (AI-inferred) */
  autoImportDataQualityResult?: boolean | Computed<boolean>;
  /** The name of the AWS Glue catalog that contains the database and table from which this data source will import metadata. (AI-inferred) */
  catalogName?: string | Computed<string>;
  /** The ARN of the IAM role that AWS DataZone assumes to access the AWS Glue data catalog and resources for this data source. (AI-inferred) */
  dataAccessRole?: string | Computed<string>;
  /** Specifies a list of relational filter configurations, each defining a database name, optional schema name, and optional filter expression to select which tables from the AWS Glue Data Catalog are ingested by the DataZone data source. (AI-inferred) */
  relationalFilterConfigurations?: DataSource_Configuration_GlueRunConfiguration_RelationalFilterConfigurations[] | Computed<DataSource_Configuration_GlueRunConfiguration_RelationalFilterConfigurations[]>;
}

export interface DataSource_Configuration_RedshiftRunConfiguration_RedshiftCredentialConfiguration {
  /** The ARN of the AWS Secrets Manager secret that stores the Redshift credentials used to connect to the Redshift data source. (AI-inferred) */
  secretManagerArn?: string | Computed<string>;
}

export interface DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorage_RedshiftClusterSource {
  /** The name of the Amazon Redshift cluster that AWS Data Zone uses as the source for ingesting data into the data source. (AI-inferred) */
  clusterName?: string | Computed<string>;
}

export interface DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorage_RedshiftServerlessSource {
  /** The name of the Redshift Serverless workgroup that serves as the data source for this Amazon Data Zone data source. (AI-inferred) */
  workgroupName?: string | Computed<string>;
}

export interface DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorage {
  /** Specifies the Amazon Redshift cluster that acts as the storage source for the data source, containing the cluster name for the Redshift run configuration. (AI-inferred) */
  redshiftClusterSource?: DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorage_RedshiftClusterSource | Computed<DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorage_RedshiftClusterSource>;
  /** Specifies the Amazon Redshift Serverless workgroup that provides the storage for the Redshift data source, identifying the serverless workgroup by its name. (AI-inferred) */
  redshiftServerlessSource?: DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorage_RedshiftServerlessSource | Computed<DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorage_RedshiftServerlessSource>;
}

export interface DataSource_Configuration_RedshiftRunConfiguration {
  /** The IAM role (ARN) that AWS Data Zone assumes to access the Amazon Redshift instance or database during data source runs. (AI-inferred) */
  dataAccessRole?: string | Computed<string>;
  /** Configures the AWS Secrets Manager secret that stores the credentials used to connect to and query the Amazon Redshift cluster during a data source run. (AI-inferred) */
  redshiftCredentialConfiguration?: DataSource_Configuration_RedshiftRunConfiguration_RedshiftCredentialConfiguration | Computed<DataSource_Configuration_RedshiftRunConfiguration_RedshiftCredentialConfiguration>;
  /** Specifies the Amazon Redshift storage source for the DataZone data source, selecting either a Redshift cluster or a Redshift Serverless workgroup via its nested RedshiftClusterSource or RedshiftServerlessSource property. (AI-inferred) */
  redshiftStorage?: DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorage | Computed<DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorage>;
  /** Defines the list of Redshift relational filter configurations, each specifying a database, schema, table, and filter expression to control which rows are ingested during a data source run. (AI-inferred) */
  relationalFilterConfigurations?: DataSource_Configuration_GlueRunConfiguration_RelationalFilterConfigurations[] | Computed<DataSource_Configuration_GlueRunConfiguration_RelationalFilterConfigurations[]>;
}

export interface DataSource_Configuration_SageMakerRunConfiguration {
  /** Specifies the list of SageMaker asset types (e.g., SageMakerModelQualityJobDefinition) whose metadata this DataZone data source will track and ingest from SageMaker runs. (AI-inferred) */
  trackingAssets?: unknown | Computed<unknown>;
}

export interface DataSource_Configuration {
  /** Specifies the AWS Glue database and optional filter expression used to discover and ingest data from Glue tables into the DataZone data source. (AI-inferred) */
  glueRunConfiguration?: DataSource_Configuration_GlueRunConfiguration | Computed<DataSource_Configuration_GlueRunConfiguration>;
  /** Specifies the Redshift-specific settings (such as the IAM role, credential and storage configuration, and relational filter configurations) used when AWS DataZone runs this data source against an Amazon Redshift cluster. (AI-inferred) */
  redshiftRunConfiguration?: DataSource_Configuration_RedshiftRunConfiguration | Computed<DataSource_Configuration_RedshiftRunConfiguration>;
  /** Specifies the SageMaker tracking assets (ARNs) that this Amazon DataZone data source will run against to discover and import metadata from SageMaker Studio. (AI-inferred) */
  sageMakerRunConfiguration?: DataSource_Configuration_SageMakerRunConfiguration | Computed<DataSource_Configuration_SageMakerRunConfiguration>;
}

export interface DataSource_Recommendation {
  /** Specifies whether automatic business name generation is to be enabled or not as part of the recommendation configuration. */
  enableBusinessNameGeneration?: boolean | Computed<boolean>;
}

export interface DataSource_Schedule {
  /** The schedule of the data source runs. */
  schedule?: string | Computed<string>;
  /** The IANA timezone name (e.g., 'America/New_York') used to interpret the cron expression for the data source's scheduled refresh. (AI-inferred) */
  timezone?: string | Computed<string>;
}

const DataSource_AssetFormsInputFields: FieldMap = {
  content: "content",
  formName: "form_name",
  typeIdentifier: "type_identifier",
  typeRevision: "type_revision",
};

const DataSource_Configuration_GlueRunConfiguration_RelationalFilterConfigurations_FilterExpressionsFields: FieldMap = {
  expression: "expression",
  type: "type",
};

const DataSource_Configuration_GlueRunConfiguration_RelationalFilterConfigurationsFields: FieldMap = {
  databaseName: "database_name",
  filterExpressions: {
    wireName: "filter_expressions",
    kind: "list",
    fields: DataSource_Configuration_GlueRunConfiguration_RelationalFilterConfigurations_FilterExpressionsFields,
  },
  schemaName: "schema_name",
};

const DataSource_Configuration_GlueRunConfigurationFields: FieldMap = {
  autoImportDataQualityResult: "auto_import_data_quality_result",
  catalogName: "catalog_name",
  dataAccessRole: "data_access_role",
  relationalFilterConfigurations: {
    wireName: "relational_filter_configurations",
    kind: "list",
    fields: DataSource_Configuration_GlueRunConfiguration_RelationalFilterConfigurationsFields,
  },
};

const DataSource_Configuration_RedshiftRunConfiguration_RedshiftCredentialConfigurationFields: FieldMap = {
  secretManagerArn: "secret_manager_arn",
};

const DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorage_RedshiftClusterSourceFields: FieldMap = {
  clusterName: "cluster_name",
};

const DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorage_RedshiftServerlessSourceFields: FieldMap = {
  workgroupName: "workgroup_name",
};

const DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorageFields: FieldMap = {
  redshiftClusterSource: {
    wireName: "redshift_cluster_source",
    kind: "object",
    fields: DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorage_RedshiftClusterSourceFields,
  },
  redshiftServerlessSource: {
    wireName: "redshift_serverless_source",
    kind: "object",
    fields: DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorage_RedshiftServerlessSourceFields,
  },
};

const DataSource_Configuration_RedshiftRunConfigurationFields: FieldMap = {
  dataAccessRole: "data_access_role",
  redshiftCredentialConfiguration: {
    wireName: "redshift_credential_configuration",
    kind: "object",
    fields: DataSource_Configuration_RedshiftRunConfiguration_RedshiftCredentialConfigurationFields,
  },
  redshiftStorage: {
    wireName: "redshift_storage",
    kind: "object",
    fields: DataSource_Configuration_RedshiftRunConfiguration_RedshiftStorageFields,
  },
  relationalFilterConfigurations: {
    wireName: "relational_filter_configurations",
    kind: "list",
    fields: DataSource_Configuration_GlueRunConfiguration_RelationalFilterConfigurationsFields,
  },
};

const DataSource_Configuration_SageMakerRunConfigurationFields: FieldMap = {
  trackingAssets: "tracking_assets",
};

const DataSource_ConfigurationFields: FieldMap = {
  glueRunConfiguration: {
    wireName: "glue_run_configuration",
    kind: "object",
    fields: DataSource_Configuration_GlueRunConfigurationFields,
  },
  redshiftRunConfiguration: {
    wireName: "redshift_run_configuration",
    kind: "object",
    fields: DataSource_Configuration_RedshiftRunConfigurationFields,
  },
  sageMakerRunConfiguration: {
    wireName: "sage_maker_run_configuration",
    kind: "object",
    fields: DataSource_Configuration_SageMakerRunConfigurationFields,
  },
};

const DataSource_RecommendationFields: FieldMap = {
  enableBusinessNameGeneration: "enable_business_name_generation",
};

const DataSource_ScheduleFields: FieldMap = {
  schedule: "schedule",
  timezone: "timezone",
};

export interface DataSourceConfig {
  /** The metadata forms that are to be attached to the assets that this data source works with. */
  assetFormsInput?: DataSource_AssetFormsInput[] | Computed<DataSource_AssetFormsInput[]>;
  /** Specifies the configuration of the data source. It can be set to either glueRunConfiguration or redshiftRunConfiguration or sageMakerRunConfiguration. */
  configuration?: DataSource_Configuration | Computed<DataSource_Configuration>;
  /** The unique identifier of a connection used to fetch relevant parameters from connection during Datasource run */
  connectionIdentifier?: string | Computed<string>;
  /** The description of the data source. */
  description?: string | Computed<string>;
  /** The ID of the Amazon DataZone domain where the data source is created. */
  domainIdentifier: string | Computed<string>;
  /** Specifies whether the data source is enabled. */
  enableSetting?: string | Computed<string>;
  /** The unique identifier of the Amazon DataZone environment to which the data source publishes assets. */
  environmentIdentifier?: string | Computed<string>;
  /** The name of the data source. */
  name: string | Computed<string>;
  /** The identifier of the Amazon DataZone project in which you want to add the data source. */
  projectIdentifier: string | Computed<string>;
  /** Specifies whether the assets that this data source creates in the inventory are to be also automatically published to the catalog. */
  publishOnImport?: boolean | Computed<boolean>;
  /** The recommendation to be updated as part of the UpdateDataSource action. */
  recommendation?: DataSource_Recommendation | Computed<DataSource_Recommendation>;
  /** The schedule of the data source runs. */
  schedule?: DataSource_Schedule | Computed<DataSource_Schedule>;
  /** The type of the data source. */
  type: string | Computed<string>;
}

export interface DataSourceAttrs {
  /** The metadata forms that are to be attached to the assets that this data source works with. */
  assetFormsInput: DataSource_AssetFormsInput[];
  /** Specifies the configuration of the data source. It can be set to either glueRunConfiguration or redshiftRunConfiguration or sageMakerRunConfiguration. */
  configuration: DataSource_Configuration;
  /** The unique identifier of a connection used to fetch relevant parameters from connection during Datasource run */
  connectionId: string;
  /** The unique identifier of a connection used to fetch relevant parameters from connection during Datasource run */
  connectionIdentifier: string;
  /** The timestamp of when the data source was created. */
  createdAt: string;
  /** The description of the data source. */
  description: string;
  /** The ID of the Amazon DataZone domain where the data source is created. */
  domainId: string;
  /** The ID of the Amazon DataZone domain where the data source is created. */
  domainIdentifier: string;
  /** Specifies whether the data source is enabled. */
  enableSetting: string;
  /** The unique identifier of the Amazon DataZone environment to which the data source publishes assets. */
  environmentId: string;
  /** The unique identifier of the Amazon DataZone environment to which the data source publishes assets. */
  environmentIdentifier: string;
  /** The unique identifier of the data source. */
  id: string;
  /** The number of assets created by the data source during its last run. */
  lastRunAssetCount: number;
  /** The timestamp that specifies when the data source was last run. */
  lastRunAt: string;
  /** The status of the last run of this data source. */
  lastRunStatus: string;
  /** The name of the data source. */
  name: string;
  /** The ID of the Amazon DataZone project to which the data source is added. */
  projectId: string;
  /** The identifier of the Amazon DataZone project in which you want to add the data source. */
  projectIdentifier: string;
  /** Specifies whether the assets that this data source creates in the inventory are to be also automatically published to the catalog. */
  publishOnImport: boolean;
  /** The recommendation to be updated as part of the UpdateDataSource action. */
  recommendation: DataSource_Recommendation;
  /** The schedule of the data source runs. */
  schedule: DataSource_Schedule;
  /** The status of the data source. */
  status: string;
  /** The type of the data source. */
  type: string;
  /** The timestamp of when this data source was updated. */
  updatedAt: string;
}

export const DataSource: ResourceBinding<DataSourceConfig, DataSourceAttrs> = {
  wireType: "aws_data_zone_data_source",
  fields: {
    assetFormsInput: {
      wireName: "asset_forms_input",
      kind: "list",
      fields: DataSource_AssetFormsInputFields,
    },
    configuration: {
      wireName: "configuration",
      kind: "object",
      fields: DataSource_ConfigurationFields,
    },
    connectionIdentifier: "connection_identifier",
    description: "description",
    domainIdentifier: "domain_identifier",
    enableSetting: "enable_setting",
    environmentIdentifier: "environment_identifier",
    name: "name",
    projectIdentifier: "project_identifier",
    publishOnImport: "publish_on_import",
    recommendation: {
      wireName: "recommendation",
      kind: "object",
      fields: DataSource_RecommendationFields,
    },
    schedule: {
      wireName: "schedule",
      kind: "object",
      fields: DataSource_ScheduleFields,
    },
    type: "type",
  },
};
