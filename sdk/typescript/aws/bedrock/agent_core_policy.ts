// Code generated by `ubx sdk gen` from aws_bedrock_agent_core_policy. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface AgentCorePolicy_Definition_Cedar {
  /** The Cedar policy statement that defines the authorization logic. */
  statement: string | Computed<string>;
}

export interface AgentCorePolicy_Definition {
  /** A Cedar policy statement within the AgentCore Policy system. */
  cedar?: AgentCorePolicy_Definition_Cedar | Computed<AgentCorePolicy_Definition_Cedar>;
  /** A policy statement within the AgentCore Policy system. */
  policy?: AgentCorePolicy_Definition_Cedar | Computed<AgentCorePolicy_Definition_Cedar>;
}

const AgentCorePolicy_Definition_CedarFields: FieldMap = {
  statement: "statement",
};

const AgentCorePolicy_DefinitionFields: FieldMap = {
  cedar: {
    wireName: "cedar",
    kind: "object",
    fields: AgentCorePolicy_Definition_CedarFields,
  },
  policy: {
    wireName: "policy",
    kind: "object",
    fields: AgentCorePolicy_Definition_CedarFields,
  },
};

export interface AgentCorePolicyConfig {
  /** The definition structure for policies. Encapsulates different policy formats. */
  definition: AgentCorePolicy_Definition | Computed<AgentCorePolicy_Definition>;
  /** A human-readable description of the policy's purpose and functionality. */
  description?: string | Computed<string>;
  /** Whether the policy contributes to the enforce decision returned to Gateway. LOG_ONLY policies are still evaluated but their decisions are observed only, allowing customers to validate a policy against real traffic before promoting it. */
  enforcementMode?: string | Computed<string>;
  /** The customer-assigned immutable name for the policy. Must be unique within the policy engine. */
  name: string | Computed<string>;
  /** The identifier of the policy engine which contains this policy. */
  policyEngineId: string | Computed<string>;
  /** The validation mode for the policy. Determines how Cedar analyzer validation results are handled. */
  validationMode?: string | Computed<string>;
}

export interface AgentCorePolicyAttrs {
  /** The timestamp when the policy was created. */
  createdAt: string;
  /** The definition structure for policies. Encapsulates different policy formats. */
  definition: AgentCorePolicy_Definition;
  /** A human-readable description of the policy's purpose and functionality. */
  description: string;
  /** Whether the policy contributes to the enforce decision returned to Gateway. LOG_ONLY policies are still evaluated but their decisions are observed only, allowing customers to validate a policy against real traffic before promoting it. */
  enforcementMode: string;
  /** The customer-assigned immutable name for the policy. Must be unique within the policy engine. */
  name: string;
  /** The Amazon Resource Name (ARN) of the policy. */
  policyArn: string;
  /** The identifier of the policy engine which contains this policy. */
  policyEngineId: string;
  /** The unique identifier for the policy. */
  policyId: string;
  status: string;
  /** Additional information about the policy status. */
  statusReasons: string[];
  /** The timestamp when the policy was last updated. */
  updatedAt: string;
  /** The validation mode for the policy. Determines how Cedar analyzer validation results are handled. */
  validationMode: string;
}

export const AgentCorePolicy: ResourceBinding<AgentCorePolicyConfig, AgentCorePolicyAttrs> = {
  wireType: "aws_bedrock_agent_core_policy",
  fields: {
    definition: {
      wireName: "definition",
      kind: "object",
      fields: AgentCorePolicy_DefinitionFields,
    },
    description: "description",
    enforcementMode: "enforcement_mode",
    name: "name",
    policyEngineId: "policy_engine_id",
    validationMode: "validation_mode",
  },
};
