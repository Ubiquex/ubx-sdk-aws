// Code generated by `ubx sdk gen` from aws_bedrock_agent_core_policy_engine. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface AgentCorePolicyEngine_Tags {
  key?: string | Computed<string>;
  value?: string | Computed<string>;
}

const AgentCorePolicyEngine_TagsFields: FieldMap = {
  key: "key",
  value: "value",
};

export interface AgentCorePolicyEngineConfig {
  /** A human-readable description of the policy engine's purpose and scope */
  description?: string | Computed<string>;
  /** The ARN of the KMS key used to encrypt the policy engine data */
  encryptionKeyArn?: string | Computed<string>;
  /** The customer-assigned immutable name for the policy engine */
  name: string | Computed<string>;
  /** A list of tags to assign to the policy engine. */
  tags?: AgentCorePolicyEngine_Tags[] | Computed<AgentCorePolicyEngine_Tags[]>;
}

export interface AgentCorePolicyEngineAttrs {
  /** The timestamp when the policy engine was created */
  createdAt: string;
  /** A human-readable description of the policy engine's purpose and scope */
  description: string;
  /** The ARN of the KMS key used to encrypt the policy engine data */
  encryptionKeyArn: string;
  /** The customer-assigned immutable name for the policy engine */
  name: string;
  /** The Amazon Resource Name (ARN) of the policy engine */
  policyEngineArn: string;
  /** The unique identifier for the policy engine */
  policyEngineId: string;
  status: string;
  /** Additional information about the policy engine status */
  statusReasons: string[];
  /** A list of tags to assign to the policy engine. */
  tags: AgentCorePolicyEngine_Tags[];
  /** The timestamp when the policy engine was last updated */
  updatedAt: string;
}

export const AgentCorePolicyEngine: ResourceBinding<AgentCorePolicyEngineConfig, AgentCorePolicyEngineAttrs> = {
  wireType: "aws_bedrock_agent_core_policy_engine",
  fields: {
    description: "description",
    encryptionKeyArn: "encryption_key_arn",
    name: "name",
    tags: {
      wireName: "tags",
      kind: "list",
      fields: AgentCorePolicyEngine_TagsFields,
    },
  },
};
