// Code generated by `ubx sdk gen` from aws_kinesis_video_signaling_channel. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface SignalingChannel_Tags {
  /** The key of a tag that you can attach to the Kinesis Video Signaling Channel, enabling you to categorize and filter the channel by user-defined metadata. (AI-inferred) */
  key?: string | Computed<string>;
  /** The value portion of a user-defined tag attached to the signaling channel, allowing you to store arbitrary metadata (e.g., names, owners, or environments) for resource management and cost allocation. (AI-inferred) */
  value?: string | Computed<string>;
}

const SignalingChannel_TagsFields: FieldMap = {
  key: "key",
  value: "value",
};

export interface SignalingChannelConfig {
  /** The period of time a signaling channel retains undelivered messages before they are discarded. */
  messageTtlSeconds?: number | Computed<number>;
  /** The name of the Kinesis Video Signaling Channel. */
  name?: string | Computed<string>;
  /** An array of key-value pairs to apply to this resource. */
  tags?: SignalingChannel_Tags[] | Computed<SignalingChannel_Tags[]>;
  /** The type of the Kinesis Video Signaling Channel to create. Currently, SINGLE_MASTER is the only supported channel type. */
  type?: string | Computed<string>;
}

export interface SignalingChannelAttrs {
  /** The Amazon Resource Name (ARN) of the Kinesis Video Signaling Channel. */
  arn: string;
  /** The period of time a signaling channel retains undelivered messages before they are discarded. */
  messageTtlSeconds: number;
  /** The name of the Kinesis Video Signaling Channel. */
  name: string;
  /** An array of key-value pairs to apply to this resource. */
  tags: SignalingChannel_Tags[];
  /** The type of the Kinesis Video Signaling Channel to create. Currently, SINGLE_MASTER is the only supported channel type. */
  type: string;
}

export const SignalingChannel: ResourceBinding<SignalingChannelConfig, SignalingChannelAttrs> = {
  wireType: "aws_kinesis_video_signaling_channel",
  fields: {
    messageTtlSeconds: "message_ttl_seconds",
    name: "name",
    tags: {
      wireName: "tags",
      kind: "list",
      fields: SignalingChannel_TagsFields,
    },
    type: "type",
  },
};
