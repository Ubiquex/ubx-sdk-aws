// Code generated by `ubx sdk gen` from aws_io_t_stream. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface Stream_Files_S3Location {
  /** The name of the S3 bucket that contains the file referenced by this IoT stream file entry. (AI-inferred) */
  bucket?: string | Computed<string>;
  /** Specifies the S3 object key (path) of the file to be added to the AWS IoT stream, locating the object within the bucket defined by the same s3_location block. (AI-inferred) */
  key?: string | Computed<string>;
  /** The version ID of the S3 object that contains the file to be streamed. (AI-inferred) */
  version?: string | Computed<string>;
}

export interface Stream_Files {
  /** The unique numeric identifier for this file within the AWS IoT Stream, used by devices to reference and load the specific file from the stream. (AI-inferred) */
  fileId?: number | Computed<number>;
  /** Specifies the Amazon S3 location (bucket, key, and optional version) of a file to be added to the AWS IoT stream, used to retrieve the file content for the stream. (AI-inferred) */
  s3Location?: Stream_Files_S3Location | Computed<Stream_Files_S3Location>;
}

export interface Stream_Tags {
  key?: string | Computed<string>;
  value?: string | Computed<string>;
}

const Stream_Files_S3LocationFields: FieldMap = {
  bucket: "bucket",
  key: "key",
  version: "version",
};

const Stream_FilesFields: FieldMap = {
  fileId: "file_id",
  s3Location: {
    wireName: "s3_location",
    kind: "object",
    fields: Stream_Files_S3LocationFields,
  },
};

const Stream_TagsFields: FieldMap = {
  key: "key",
  value: "value",
};

export interface StreamConfig {
  /** The description of the stream. */
  description?: string | Computed<string>;
  /** The files to stream. */
  files: Stream_Files[] | Computed<Stream_Files[]>;
  /** An IAM role that allows the IoT service principal to access your S3 files. */
  roleArn: string | Computed<string>;
  /** The stream ID. */
  streamId: string | Computed<string>;
  /** Metadata which can be used to manage streams. */
  tags?: Stream_Tags[] | Computed<Stream_Tags[]>;
}

export interface StreamAttrs {
  /** The Amazon Resource Name (ARN) of the stream. */
  arn: string;
  /** The date when the stream was created. */
  createdAt: string;
  /** The description of the stream. */
  description: string;
  /** The files to stream. */
  files: Stream_Files[];
  /** The date when the stream was last updated. */
  lastUpdatedAt: string;
  /** An IAM role that allows the IoT service principal to access your S3 files. */
  roleArn: string;
  /** The stream ID. */
  streamId: string;
  /** The stream version. */
  streamVersion: number;
  /** Metadata which can be used to manage streams. */
  tags: Stream_Tags[];
}

export const Stream: ResourceBinding<StreamConfig, StreamAttrs> = {
  wireType: "aws_io_t_stream",
  fields: {
    description: "description",
    files: {
      wireName: "files",
      kind: "list",
      fields: Stream_FilesFields,
    },
    roleArn: "role_arn",
    streamId: "stream_id",
    tags: {
      wireName: "tags",
      kind: "list",
      fields: Stream_TagsFields,
    },
  },
};
