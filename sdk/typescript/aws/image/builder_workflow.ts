// Code generated by `ubx sdk gen` from aws_image_builder_workflow. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface BuilderWorkflow_LatestVersion {
  /** The latest version ARN of the created workflow. */
  arn?: string | Computed<string>;
  /** The latest version ARN of the created workflow, with the same major version. */
  major?: string | Computed<string>;
  /** The latest version ARN of the created workflow, with the same minor version. */
  minor?: string | Computed<string>;
  /** The latest version ARN of the created workflow, with the same patch version. */
  patch?: string | Computed<string>;
}

export interface BuilderWorkflowConfig {
  /** The change description of the workflow. */
  changeDescription?: string | Computed<string>;
  /** The data of the workflow. */
  data?: string | Computed<string>;
  /** The description of the workflow. */
  description?: string | Computed<string>;
  /** The KMS key identifier used to encrypt the workflow. */
  kmsKeyId?: string | Computed<string>;
  /** The name of the workflow. */
  name: string | Computed<string>;
  /** The tags associated with the workflow. */
  tags?: unknown | Computed<unknown>;
  /** The type of the workflow denotes whether the workflow is used to build, test, or distribute. */
  type: string | Computed<string>;
  /** The uri of the workflow. */
  uri?: string | Computed<string>;
  /** The version of the workflow. */
  version: string | Computed<string>;
}

export interface BuilderWorkflowAttrs {
  /** The Amazon Resource Name (ARN) of the workflow. */
  arn: string;
  /** The change description of the workflow. */
  changeDescription: string;
  /** The data of the workflow. */
  data: string;
  /** The description of the workflow. */
  description: string;
  /** The KMS key identifier used to encrypt the workflow. */
  kmsKeyId: string;
  /** The latest version references of the workflow. */
  latestVersion: BuilderWorkflow_LatestVersion;
  /** The name of the workflow. */
  name: string;
  /** The tags associated with the workflow. */
  tags: unknown;
  /** The type of the workflow denotes whether the workflow is used to build, test, or distribute. */
  type: string;
  /** The uri of the workflow. */
  uri: string;
  /** The version of the workflow. */
  version: string;
}

export const BuilderWorkflow: ResourceBinding<BuilderWorkflowConfig, BuilderWorkflowAttrs> = {
  wireType: "aws_image_builder_workflow",
  fields: {
    changeDescription: "change_description",
    data: "data",
    description: "description",
    kmsKeyId: "kms_key_id",
    name: "name",
    tags: "tags",
    type: "type",
    uri: "uri",
    version: "version",
  },
};
