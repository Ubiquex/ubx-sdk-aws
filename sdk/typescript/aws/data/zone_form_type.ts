// Code generated by `ubx sdk gen` from aws_data_zone_form_type. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface ZoneFormType_Model {
  /** The Smithy model definition that specifies the structure and validation rules for the form type. (AI-inferred) */
  smithy?: string | Computed<string>;
}

const ZoneFormType_ModelFields: FieldMap = {
  smithy: "smithy",
};

export interface ZoneFormTypeConfig {
  /** The description of this Amazon DataZone metadata form type. */
  description?: string | Computed<string>;
  /** The ID of the Amazon DataZone domain in which this metadata form type is created. */
  domainIdentifier: string | Computed<string>;
  /** Indicates the smithy model of the API. */
  model: ZoneFormType_Model | Computed<ZoneFormType_Model>;
  /** The name of this Amazon DataZone metadata form type. */
  name: string | Computed<string>;
  /** The ID of the Amazon DataZone project that owns this metadata form type. */
  owningProjectIdentifier: string | Computed<string>;
  /** The status of this Amazon DataZone metadata form type. */
  status?: string | Computed<string>;
}

export interface ZoneFormTypeAttrs {
  /** The timestamp of when this Amazon DataZone metadata form type was created. */
  createdAt: string;
  /** The user who created this Amazon DataZone metadata form type. */
  createdBy: string;
  /** The description of this Amazon DataZone metadata form type. */
  description: string;
  /** The ID of the Amazon DataZone domain in which this metadata form type is created. */
  domainId: string;
  /** The ID of the Amazon DataZone domain in which this metadata form type is created. */
  domainIdentifier: string;
  /** The ID of this Amazon DataZone metadata form type. */
  formTypeIdentifier: string;
  /** Indicates the smithy model of the API. */
  model: ZoneFormType_Model;
  /** The name of this Amazon DataZone metadata form type. */
  name: string;
  /** The ID of the project that owns this Amazon DataZone metadata form type. */
  owningProjectId: string;
  /** The ID of the Amazon DataZone project that owns this metadata form type. */
  owningProjectIdentifier: string;
  /** The revision of this Amazon DataZone metadata form type. */
  revision: string;
  /** The status of this Amazon DataZone metadata form type. */
  status: string;
}

export const ZoneFormType: ResourceBinding<ZoneFormTypeConfig, ZoneFormTypeAttrs> = {
  wireType: "aws_data_zone_form_type",
  fields: {
    description: "description",
    domainIdentifier: "domain_identifier",
    model: {
      wireName: "model",
      kind: "object",
      fields: ZoneFormType_ModelFields,
    },
    name: "name",
    owningProjectIdentifier: "owning_project_identifier",
    status: "status",
  },
};
