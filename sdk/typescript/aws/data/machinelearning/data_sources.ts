// Code generated by `ubx sdk gen` from aws_machinelearning_data_sources. DO NOT EDIT.
import type { Computed, FieldMap, DataSourceBinding } from "@ubx/sdk";

export interface DataSources_Results_Rdsmetadata_Database {
  databaseName?: string | Computed<string>;
  instanceIdentifier?: string | Computed<string>;
}

export interface DataSources_Results_Rdsmetadata {
  dataPipelineId?: string | Computed<string>;
  database?: DataSources_Results_Rdsmetadata_Database | Computed<DataSources_Results_Rdsmetadata_Database>;
  databaseUserName?: string | Computed<string>;
  resourceRole?: string | Computed<string>;
  selectSqlQuery?: string | Computed<string>;
  serviceRole?: string | Computed<string>;
}

export interface DataSources_Results_RedshiftMetadata_RedshiftDatabase {
  clusterIdentifier?: string | Computed<string>;
  databaseName?: string | Computed<string>;
}

export interface DataSources_Results_RedshiftMetadata {
  databaseUserName?: string | Computed<string>;
  redshiftDatabase?: DataSources_Results_RedshiftMetadata_RedshiftDatabase | Computed<DataSources_Results_RedshiftMetadata_RedshiftDatabase>;
  selectSqlQuery?: string | Computed<string>;
}

export interface DataSources_Results {
  computeStatistics?: boolean | Computed<boolean>;
  computeTime?: number | Computed<number>;
  createdAt?: string | Computed<string>;
  createdByIamUser?: string | Computed<string>;
  dataLocationS3?: string | Computed<string>;
  dataRearrangement?: string | Computed<string>;
  dataSizeInBytes?: number | Computed<number>;
  dataSourceId?: string | Computed<string>;
  finishedAt?: string | Computed<string>;
  lastUpdatedAt?: string | Computed<string>;
  message?: string | Computed<string>;
  name?: string | Computed<string>;
  numberOfFiles?: number | Computed<number>;
  rdsmetadata?: DataSources_Results_Rdsmetadata | Computed<DataSources_Results_Rdsmetadata>;
  redshiftMetadata?: DataSources_Results_RedshiftMetadata | Computed<DataSources_Results_RedshiftMetadata>;
  roleArn?: string | Computed<string>;
  startedAt?: string | Computed<string>;
  status?: string | Computed<string>;
}

export interface DataSourcesConfig {
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  eq?: string | Computed<string>;
  /** <p>A list of the variables to use in searching or filtering <code>DataSource</code>.</p> <ul> <li> <p> <code>CreatedAt</code> - Sets the search criteria to <code>DataSource</code> creation date.</p> </li> <li> <p> <code>Status</code> - Sets the search criteria to <code>DataSource</code> status.</p> </li> <li> <p> <code>Name</code> - Sets the search criteria to the contents of <code>DataSource</code> <code>Name</code>.</p> </li> <li> <p> <code>DataUri</code> - Sets the search criteria to the URI of data files used to create the <code>DataSource</code>. The URI can identify either a file or an Amazon Simple Storage Service (Amazon S3) bucket or directory.</p> </li> <li> <p> <code>IAMUser</code> - Sets the search criteria to the user account that invoked the <code>DataSource</code> creation.</p> </li> </ul> <p> <b>Note:</b> The variable names should match the variable names in the <code>DataSource</code>.</p> */
  filterVariable?: string | Computed<string>;
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  ge?: string | Computed<string>;
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  gt?: string | Computed<string>;
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  le?: string | Computed<string>;
  limit?: number | Computed<number>;
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  lt?: string | Computed<string>;
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  ne?: string | Computed<string>;
  /** <p>String type.</p> */
  nextToken?: string | Computed<string>;
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  prefix?: string | Computed<string>;
  /** <p>The sort order specified in a listing condition. Possible values include the following:</p> <ul> <li> <p> <code>asc</code> - Present the information in ascending order (from A-Z).</p> </li> <li> <p> <code>dsc</code> - Present the information in descending order (from Z-A).</p> </li> </ul> */
  sortOrder?: string | Computed<string>;
}

export interface DataSourcesAttrs {
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  eq: string;
  /** <p>A list of the variables to use in searching or filtering <code>DataSource</code>.</p> <ul> <li> <p> <code>CreatedAt</code> - Sets the search criteria to <code>DataSource</code> creation date.</p> </li> <li> <p> <code>Status</code> - Sets the search criteria to <code>DataSource</code> status.</p> </li> <li> <p> <code>Name</code> - Sets the search criteria to the contents of <code>DataSource</code> <code>Name</code>.</p> </li> <li> <p> <code>DataUri</code> - Sets the search criteria to the URI of data files used to create the <code>DataSource</code>. The URI can identify either a file or an Amazon Simple Storage Service (Amazon S3) bucket or directory.</p> </li> <li> <p> <code>IAMUser</code> - Sets the search criteria to the user account that invoked the <code>DataSource</code> creation.</p> </li> </ul> <p> <b>Note:</b> The variable names should match the variable names in the <code>DataSource</code>.</p> */
  filterVariable: string;
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  ge: string;
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  gt: string;
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  le: string;
  limit: number;
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  lt: string;
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  ne: string;
  /** <p>String type.</p> */
  nextToken: string;
  /** <p>The value specified in a filtering condition. The <code>ComparatorValue</code> becomes the reference value when matching or evaluating data values in filtering and searching functions.</p> */
  prefix: string;
  results: DataSources_Results[];
  /** <p>The sort order specified in a listing condition. Possible values include the following:</p> <ul> <li> <p> <code>asc</code> - Present the information in ascending order (from A-Z).</p> </li> <li> <p> <code>dsc</code> - Present the information in descending order (from Z-A).</p> </li> </ul> */
  sortOrder: string;
}

export const DataSources: DataSourceBinding<DataSourcesConfig, DataSourcesAttrs> = {
  wireType: "aws_machinelearning_data_sources",
  fields: {
    eq: "eq",
    filterVariable: "filter_variable",
    ge: "ge",
    gt: "gt",
    le: "le",
    limit: "limit",
    lt: "lt",
    ne: "ne",
    nextToken: "next_token",
    prefix: "prefix",
    sortOrder: "sort_order",
  },
};
