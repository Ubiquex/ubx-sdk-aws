# Code generated by `ubx sdk gen` from aws_guard_duty_publishing_destination. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class PublishingDestination_DestinationProperties:
    # The ARN of the resource to publish to.
    destination_arn: Any = None
    # The ARN of the KMS key to use for encryption.
    kms_key_arn: Any = None

@dataclasses.dataclass
class PublishingDestination_Tags:
    # The key of a tag assigned to the GuardDuty publishing destination, used to organize and identify the resource. (AI-inferred)
    key: Any = None
    # The value portion of a user-defined tag that is associated with the GuardDuty publishing destination, used for resource organization, cost tracking, and management. (AI-inferred)
    value: Any = None

_PublishingDestination_DestinationPropertiesFields = {
    "destination_arn": ubx.FieldSpec(wire_name="destination_arn"),
    "kms_key_arn": ubx.FieldSpec(wire_name="kms_key_arn"),
}

_PublishingDestination_TagsFields = {
    "key": ubx.FieldSpec(wire_name="key"),
    "value": ubx.FieldSpec(wire_name="value"),
}

@dataclasses.dataclass
class PublishingDestinationConfig:
    # Configures the GuardDuty publishing destination by specifying the ARN of the Amazon S3 bucket to receive findings and the ARN of the AWS KMS key used to encrypt those findings. (AI-inferred)
    destination_properties: Any = None
    # The type of resource for the publishing destination. Currently only Amazon S3 buckets are supported.
    destination_type: Any = None
    # The ID of the GuardDuty detector associated with the publishing destination.
    detector_id: Any = None
    # A list of key-value tags to attach to the GuardDuty publishing destination for organizing and identifying the resource. (AI-inferred)
    tags: Any = None

@dataclasses.dataclass
class PublishingDestinationAttrs:
    # Configures the GuardDuty publishing destination by specifying the ARN of the Amazon S3 bucket to receive findings and the ARN of the AWS KMS key used to encrypt those findings. (AI-inferred)
    destination_properties: Any = None
    # The type of resource for the publishing destination. Currently only Amazon S3 buckets are supported.
    destination_type: Any = None
    # The ID of the GuardDuty detector associated with the publishing destination.
    detector_id: Any = None
    # The ID of the publishing destination.
    id: Any = None
    # The time, in epoch millisecond format, at which GuardDuty was first unable to publish findings to the destination.
    publishing_failure_start_timestamp: Any = None
    # The status of the publishing destination.
    status: Any = None
    # A list of key-value tags to attach to the GuardDuty publishing destination for organizing and identifying the resource. (AI-inferred)
    tags: Any = None

PublishingDestination = ubx.ResourceBinding(
    wire_type="aws_guard_duty_publishing_destination",
    fields={
        "destination_properties": ubx.FieldSpec(
            wire_name="destination_properties",
            kind="object",
            fields=_PublishingDestination_DestinationPropertiesFields,
        ),
        "destination_type": ubx.FieldSpec(wire_name="destination_type"),
        "detector_id": ubx.FieldSpec(wire_name="detector_id"),
        "tags": ubx.FieldSpec(
            wire_name="tags",
            kind="list",
            fields=_PublishingDestination_TagsFields,
        ),
    },
)
