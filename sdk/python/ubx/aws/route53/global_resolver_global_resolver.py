# Code generated by `ubx sdk gen` from aws_route53_global_resolver_global_resolver. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class GlobalResolverGlobalResolver_Tags:
    key: Any = None
    value: Any = None

_GlobalResolverGlobalResolver_TagsFields = {
    "key": ubx.FieldSpec(wire_name="key"),
    "value": ubx.FieldSpec(wire_name="value"),
}

@dataclasses.dataclass
class GlobalResolverGlobalResolverConfig:
    client_token: Any = None
    description: Any = None
    ip_address_type: Any = None
    name: Any = None
    observability_region: Any = None
    # The list of regions the Global Resolver exists in. Regions can be added or removed on update; the order of this list is not significant.
    regions: Any = None
    tags: Any = None

@dataclasses.dataclass
class GlobalResolverGlobalResolverAttrs:
    arn: Any = None
    client_token: Any = None
    created_at: Any = None
    description: Any = None
    dns_name: Any = None
    global_resolver_id: Any = None
    ipv4_addresses: Any = None
    ipv6_addresses: Any = None
    ip_address_type: Any = None
    name: Any = None
    observability_region: Any = None
    # The list of regions the Global Resolver exists in. Regions can be added or removed on update; the order of this list is not significant.
    regions: Any = None
    status: Any = None
    tags: Any = None
    updated_at: Any = None

GlobalResolverGlobalResolver = ubx.ResourceBinding(
    wire_type="aws_route53_global_resolver_global_resolver",
    fields={
        "client_token": ubx.FieldSpec(wire_name="client_token"),
        "description": ubx.FieldSpec(wire_name="description"),
        "ip_address_type": ubx.FieldSpec(wire_name="ip_address_type"),
        "name": ubx.FieldSpec(wire_name="name"),
        "observability_region": ubx.FieldSpec(wire_name="observability_region"),
        "regions": ubx.FieldSpec(wire_name="regions"),
        "tags": ubx.FieldSpec(
            wire_name="tags",
            kind="list",
            fields=_GlobalResolverGlobalResolver_TagsFields,
        ),
    },
)
