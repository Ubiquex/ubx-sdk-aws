# Code generated by `ubx sdk gen` from aws_bedrock_agent_core_policy_engine. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class AgentCorePolicyEngine_Tags:
    key: Any = None
    value: Any = None

_AgentCorePolicyEngine_TagsFields = {
    "key": ubx.FieldSpec(wire_name="key"),
    "value": ubx.FieldSpec(wire_name="value"),
}

@dataclasses.dataclass
class AgentCorePolicyEngineConfig:
    # A human-readable description of the policy engine's purpose and scope
    description: Any = None
    # The ARN of the KMS key used to encrypt the policy engine data
    encryption_key_arn: Any = None
    # The customer-assigned immutable name for the policy engine
    name: Any = None
    # A list of tags to assign to the policy engine.
    tags: Any = None

@dataclasses.dataclass
class AgentCorePolicyEngineAttrs:
    # The timestamp when the policy engine was created
    created_at: Any = None
    # A human-readable description of the policy engine's purpose and scope
    description: Any = None
    # The ARN of the KMS key used to encrypt the policy engine data
    encryption_key_arn: Any = None
    # The customer-assigned immutable name for the policy engine
    name: Any = None
    # The Amazon Resource Name (ARN) of the policy engine
    policy_engine_arn: Any = None
    # The unique identifier for the policy engine
    policy_engine_id: Any = None
    status: Any = None
    # Additional information about the policy engine status
    status_reasons: Any = None
    # A list of tags to assign to the policy engine.
    tags: Any = None
    # The timestamp when the policy engine was last updated
    updated_at: Any = None

AgentCorePolicyEngine = ubx.ResourceBinding(
    wire_type="aws_bedrock_agent_core_policy_engine",
    fields={
        "description": ubx.FieldSpec(wire_name="description"),
        "encryption_key_arn": ubx.FieldSpec(wire_name="encryption_key_arn"),
        "name": ubx.FieldSpec(wire_name="name"),
        "tags": ubx.FieldSpec(
            wire_name="tags",
            kind="list",
            fields=_AgentCorePolicyEngine_TagsFields,
        ),
    },
)
