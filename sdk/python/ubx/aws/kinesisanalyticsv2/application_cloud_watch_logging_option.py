# Code generated by `ubx sdk gen` from aws_kinesis_analytics_v2_application_cloud_watch_logging_option. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class ApplicationCloudWatchLoggingOption_CloudWatchLoggingOption:
    # The ARN of the CloudWatch log stream that the Kinesis Data Analytics for Apache Flink application writes its diagnostic logs to. (AI-inferred)
    log_stream_arn: Any = None

_ApplicationCloudWatchLoggingOption_CloudWatchLoggingOptionFields = {
    "log_stream_arn": ubx.FieldSpec(wire_name="log_stream_arn"),
}

@dataclasses.dataclass
class ApplicationCloudWatchLoggingOptionConfig:
    # The name of the Kinesis Data Analytics for Apache Flink application to which this CloudWatch logging option is attached. (AI-inferred)
    application_name: Any = None
    # Configures the CloudWatch Logs log stream (via its ARN) to which the Kinesis Analytics v2 application sends operational logs and metrics. (AI-inferred)
    cloud_watch_logging_option: Any = None

@dataclasses.dataclass
class ApplicationCloudWatchLoggingOptionAttrs:
    # The name of the Kinesis Data Analytics for Apache Flink application to which this CloudWatch logging option is attached. (AI-inferred)
    application_name: Any = None
    # Configures the CloudWatch Logs log stream (via its ARN) to which the Kinesis Analytics v2 application sends operational logs and metrics. (AI-inferred)
    cloud_watch_logging_option: Any = None
    id: Any = None

ApplicationCloudWatchLoggingOption = ubx.ResourceBinding(
    wire_type="aws_kinesis_analytics_v2_application_cloud_watch_logging_option",
    fields={
        "application_name": ubx.FieldSpec(wire_name="application_name"),
        "cloud_watch_logging_option": ubx.FieldSpec(
            wire_name="cloud_watch_logging_option",
            kind="object",
            fields=_ApplicationCloudWatchLoggingOption_CloudWatchLoggingOptionFields,
        ),
    },
)
