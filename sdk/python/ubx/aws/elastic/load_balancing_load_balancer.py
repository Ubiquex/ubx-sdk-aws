# Code generated by `ubx sdk gen` from aws_elastic_load_balancing_load_balancer. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class LoadBalancingLoadBalancer_AccessLoggingPolicy:
    # The interval for publishing the access logs. You can specify an interval of either 5 minutes or 60 minutes. Default: 60 minutes
    emit_interval: Any = None
    # Specifies whether access logs are enabled for the load balancer.
    enabled: Any = None
    # The name of the Amazon S3 bucket where the access logs are stored.
    s3_bucket_name: Any = None
    # The logical hierarchy you created for your Amazon S3 bucket, for example `my-bucket-prefix/prod`. If the prefix is not provided, the log is placed at the root level of the bucket.
    s3_bucket_prefix: Any = None

@dataclasses.dataclass
class LoadBalancingLoadBalancer_AppCookieStickinessPolicy:
    cookie_name: Any = None
    policy_name: Any = None

@dataclasses.dataclass
class LoadBalancingLoadBalancer_ConnectionDrainingPolicy:
    # Specifies whether connection draining is enabled for the load balancer.
    enabled: Any = None
    # The maximum time, in seconds, to keep the existing connections open before deregistering the instances.
    timeout: Any = None

@dataclasses.dataclass
class LoadBalancingLoadBalancer_ConnectionSettings:
    # The time, in seconds, that the connection is allowed to be idle (no data has been sent over the connection) before it is closed by the load balancer.
    idle_timeout: Any = None

@dataclasses.dataclass
class LoadBalancingLoadBalancer_HealthCheck:
    # The number of consecutive health checks successes required before moving the instance to the `Healthy` state.
    healthy_threshold: Any = None
    # The approximate interval, in seconds, between health checks of an individual instance.
    interval: Any = None
    # The instance being checked.
    target: Any = None
    # The amount of time, in seconds, during which no response means a failed health check. This value must be less than the `Interval` value.
    timeout: Any = None
    # The number of consecutive health check failures required before moving the instance to the `Unhealthy` state.
    unhealthy_threshold: Any = None

@dataclasses.dataclass
class LoadBalancingLoadBalancer_LbcookieStickinessPolicy:
    cookie_expiration_period: Any = None
    policy_name: Any = None

@dataclasses.dataclass
class LoadBalancingLoadBalancer_Listeners:
    instance_port: Any = None
    instance_protocol: Any = None
    load_balancer_port: Any = None
    policy_names: Any = None
    protocol: Any = None
    sslcertificate_id: Any = None

@dataclasses.dataclass
class LoadBalancingLoadBalancer_Policies_Attributes:
    name: Any = None
    value: Any = None

@dataclasses.dataclass
class LoadBalancingLoadBalancer_Policies:
    attributes: Any = None
    instance_ports: Any = None
    load_balancer_ports: Any = None
    policy_name: Any = None
    policy_type: Any = None

@dataclasses.dataclass
class LoadBalancingLoadBalancer_SourceSecurityGroup:
    # The name of the security group that you can use as part of your inbound rules for your load balancer's back-end instances.
    group_name: Any = None
    # The owner of the source security group.
    owner_alias: Any = None

@dataclasses.dataclass
class LoadBalancingLoadBalancer_Tags:
    key: Any = None
    value: Any = None

_LoadBalancingLoadBalancer_AccessLoggingPolicyFields = {
    "emit_interval": ubx.FieldSpec(wire_name="emit_interval"),
    "enabled": ubx.FieldSpec(wire_name="enabled"),
    "s3_bucket_name": ubx.FieldSpec(wire_name="s3_bucket_name"),
    "s3_bucket_prefix": ubx.FieldSpec(wire_name="s3_bucket_prefix"),
}

_LoadBalancingLoadBalancer_AppCookieStickinessPolicyFields = {
    "cookie_name": ubx.FieldSpec(wire_name="cookie_name"),
    "policy_name": ubx.FieldSpec(wire_name="policy_name"),
}

_LoadBalancingLoadBalancer_ConnectionDrainingPolicyFields = {
    "enabled": ubx.FieldSpec(wire_name="enabled"),
    "timeout": ubx.FieldSpec(wire_name="timeout"),
}

_LoadBalancingLoadBalancer_ConnectionSettingsFields = {
    "idle_timeout": ubx.FieldSpec(wire_name="idle_timeout"),
}

_LoadBalancingLoadBalancer_HealthCheckFields = {
    "healthy_threshold": ubx.FieldSpec(wire_name="healthy_threshold"),
    "interval": ubx.FieldSpec(wire_name="interval"),
    "target": ubx.FieldSpec(wire_name="target"),
    "timeout": ubx.FieldSpec(wire_name="timeout"),
    "unhealthy_threshold": ubx.FieldSpec(wire_name="unhealthy_threshold"),
}

_LoadBalancingLoadBalancer_LbcookieStickinessPolicyFields = {
    "cookie_expiration_period": ubx.FieldSpec(wire_name="cookie_expiration_period"),
    "policy_name": ubx.FieldSpec(wire_name="policy_name"),
}

_LoadBalancingLoadBalancer_ListenersFields = {
    "instance_port": ubx.FieldSpec(wire_name="instance_port"),
    "instance_protocol": ubx.FieldSpec(wire_name="instance_protocol"),
    "load_balancer_port": ubx.FieldSpec(wire_name="load_balancer_port"),
    "policy_names": ubx.FieldSpec(wire_name="policy_names"),
    "protocol": ubx.FieldSpec(wire_name="protocol"),
    "sslcertificate_id": ubx.FieldSpec(wire_name="sslcertificate_id"),
}

_LoadBalancingLoadBalancer_Policies_AttributesFields = {
    "name": ubx.FieldSpec(wire_name="name"),
    "value": ubx.FieldSpec(wire_name="value"),
}

_LoadBalancingLoadBalancer_PoliciesFields = {
    "attributes": ubx.FieldSpec(
        wire_name="attributes",
        kind="list",
        fields=_LoadBalancingLoadBalancer_Policies_AttributesFields,
    ),
    "instance_ports": ubx.FieldSpec(wire_name="instance_ports"),
    "load_balancer_ports": ubx.FieldSpec(wire_name="load_balancer_ports"),
    "policy_name": ubx.FieldSpec(wire_name="policy_name"),
    "policy_type": ubx.FieldSpec(wire_name="policy_type"),
}

_LoadBalancingLoadBalancer_TagsFields = {
    "key": ubx.FieldSpec(wire_name="key"),
    "value": ubx.FieldSpec(wire_name="value"),
}

@dataclasses.dataclass
class LoadBalancingLoadBalancerConfig:
    # Information about where and how access logs are stored for the load balancer.
    access_logging_policy: Any = None
    # Information about a policy for application-controlled session stickiness.
    app_cookie_stickiness_policy: Any = None
    # The Availability Zones for a load balancer in a default VPC. For a load balancer in a nondefault VPC, specify Subnets instead.
    availability_zones: Any = None
    # If enabled, the load balancer allows existing requests to complete before the load balancer shifts traffic away from a deregistered or unhealthy instance.
    connection_draining_policy: Any = None
    # If enabled, the load balancer allows the connections to remain idle (no data is sent over the connection) for the specified duration.
    connection_settings: Any = None
    # If enabled, the load balancer routes the request traffic evenly across all instances regardless of the Availability Zones.
    cross_zone: Any = None
    # The health check settings to use when evaluating the health of your EC2 instances. Update requires replacement if you did not previously specify health check settings or if you are removing the health check settings. Otherwise, update requires no interruption.
    health_check: Any = None
    # The IDs of the instances for the load balancer.
    instances: Any = None
    # Information about a policy for duration-based session stickiness.
    lbcookie_stickiness_policy: Any = None
    # The Listeners for the load balancer. You can specify at most one listener per port.
    listeners: Any = None
    # The name of the load balancer. This name must be unique within your set of load balancers for the region.
    load_balancer_name: Any = None
    # The policies defined for your Classic Load Balancer. Specify only back-end server policies.
    policies: Any = None
    # The type of load balancer. Valid only for load balancers in a VPC.
    scheme: Any = None
    # The security groups for the load balancer. Valid only for load balancers in a VPC.
    security_groups: Any = None
    # The IDs of the subnets for the load balancer. You can specify at most one subnet per Availability Zone.
    subnets: Any = None
    # The tags associated with a load balancer.
    tags: Any = None

@dataclasses.dataclass
class LoadBalancingLoadBalancerAttrs:
    # Information about where and how access logs are stored for the load balancer.
    access_logging_policy: Any = None
    # Information about a policy for application-controlled session stickiness.
    app_cookie_stickiness_policy: Any = None
    # The Availability Zones for a load balancer in a default VPC. For a load balancer in a nondefault VPC, specify Subnets instead.
    availability_zones: Any = None
    # The name of the Route 53 hosted zone that is associated with the load balancer. Internal-facing load balancers.
    canonical_hosted_zone_name: Any = None
    # The ID of the Route 53 hosted zone name that is associated with the load balancer.
    canonical_hosted_zone_name_id: Any = None
    # If enabled, the load balancer allows existing requests to complete before the load balancer shifts traffic away from a deregistered or unhealthy instance.
    connection_draining_policy: Any = None
    # If enabled, the load balancer allows the connections to remain idle (no data is sent over the connection) for the specified duration.
    connection_settings: Any = None
    # If enabled, the load balancer routes the request traffic evenly across all instances regardless of the Availability Zones.
    cross_zone: Any = None
    # The DNS name for the load balancer
    dnsname: Any = None
    # The health check settings to use when evaluating the health of your EC2 instances. Update requires replacement if you did not previously specify health check settings or if you are removing the health check settings. Otherwise, update requires no interruption.
    health_check: Any = None
    # The IDs of the instances for the load balancer.
    instances: Any = None
    # Information about a policy for duration-based session stickiness.
    lbcookie_stickiness_policy: Any = None
    # The Listeners for the load balancer. You can specify at most one listener per port.
    listeners: Any = None
    # The name of the load balancer. This name must be unique within your set of load balancers for the region.
    load_balancer_name: Any = None
    # The policies defined for your Classic Load Balancer. Specify only back-end server policies.
    policies: Any = None
    # The type of load balancer. Valid only for load balancers in a VPC.
    scheme: Any = None
    # The security groups for the load balancer. Valid only for load balancers in a VPC.
    security_groups: Any = None
    source_security_group: Any = None
    # The IDs of the subnets for the load balancer. You can specify at most one subnet per Availability Zone.
    subnets: Any = None
    # The tags associated with a load balancer.
    tags: Any = None

LoadBalancingLoadBalancer = ubx.ResourceBinding(
    wire_type="aws_elastic_load_balancing_load_balancer",
    fields={
        "access_logging_policy": ubx.FieldSpec(
            wire_name="access_logging_policy",
            kind="object",
            fields=_LoadBalancingLoadBalancer_AccessLoggingPolicyFields,
        ),
        "app_cookie_stickiness_policy": ubx.FieldSpec(
            wire_name="app_cookie_stickiness_policy",
            kind="list",
            fields=_LoadBalancingLoadBalancer_AppCookieStickinessPolicyFields,
        ),
        "availability_zones": ubx.FieldSpec(wire_name="availability_zones"),
        "connection_draining_policy": ubx.FieldSpec(
            wire_name="connection_draining_policy",
            kind="object",
            fields=_LoadBalancingLoadBalancer_ConnectionDrainingPolicyFields,
        ),
        "connection_settings": ubx.FieldSpec(
            wire_name="connection_settings",
            kind="object",
            fields=_LoadBalancingLoadBalancer_ConnectionSettingsFields,
        ),
        "cross_zone": ubx.FieldSpec(wire_name="cross_zone"),
        "health_check": ubx.FieldSpec(
            wire_name="health_check",
            kind="object",
            fields=_LoadBalancingLoadBalancer_HealthCheckFields,
        ),
        "instances": ubx.FieldSpec(wire_name="instances"),
        "lbcookie_stickiness_policy": ubx.FieldSpec(
            wire_name="lbcookie_stickiness_policy",
            kind="list",
            fields=_LoadBalancingLoadBalancer_LbcookieStickinessPolicyFields,
        ),
        "listeners": ubx.FieldSpec(
            wire_name="listeners",
            kind="list",
            fields=_LoadBalancingLoadBalancer_ListenersFields,
        ),
        "load_balancer_name": ubx.FieldSpec(wire_name="load_balancer_name"),
        "policies": ubx.FieldSpec(
            wire_name="policies",
            kind="list",
            fields=_LoadBalancingLoadBalancer_PoliciesFields,
        ),
        "scheme": ubx.FieldSpec(wire_name="scheme"),
        "security_groups": ubx.FieldSpec(wire_name="security_groups"),
        "subnets": ubx.FieldSpec(wire_name="subnets"),
        "tags": ubx.FieldSpec(
            wire_name="tags",
            kind="list",
            fields=_LoadBalancingLoadBalancer_TagsFields,
        ),
    },
)
