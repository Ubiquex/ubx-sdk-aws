# Code generated by `ubx sdk gen` from aws_cloud_formation_guard_hook. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class GuardHook_RuleLocation:
    # S3 uri of Guard files.
    uri: Any = None
    # S3 object version
    version_id: Any = None

@dataclasses.dataclass
class GuardHook_StackFilters_StackNames:
    # List of stack names that the hook is going to be excluded from
    exclude: Any = None
    # List of stack names that the hook is going to target
    include: Any = None

@dataclasses.dataclass
class GuardHook_StackFilters_StackRoles:
    # List of stack roles that the hook is going to be excluded from
    exclude: Any = None
    # List of stack roles that the hook is going to target
    include: Any = None

@dataclasses.dataclass
class GuardHook_StackFilters:
    # Attribute to specify the filtering behavior. ANY will make the Hook pass if one filter matches. ALL will make the Hook pass if all filters match
    filtering_criteria: Any = None
    # List of stack names as filters
    stack_names: Any = None
    # List of stack roles that are performing the stack operations.
    stack_roles: Any = None

@dataclasses.dataclass
class GuardHook_TargetFilters_Targets:
    # Specifies whether the Guard hook should be invoked (INVOKE) or skipped (SKIP) for resources matching the target in this target filter. (AI-inferred)
    action: Any = None
    # The invocation point for a target in the Guard hook's target filter, indicating which CloudFormation resource lifecycle event (CREATE, UPDATE, or DELETE) triggers the hook for that target. (AI-inferred)
    invocation_point: Any = None
    # The name of a target resource type (for example, AWS::S3::Bucket) that this Guard hook filter matches so the hook is invoked only for resources of that type. (AI-inferred)
    target_name: Any = None

@dataclasses.dataclass
class GuardHook_TargetFilters:
    # Specifies the list of stack operations (CREATE, UPDATE, DELETE) for which the Guard hook is invoked on the targeted resources. (AI-inferred)
    actions: Any = None
    # The invocation points of the target filter, which are the stack lifecycle events (such as BEFORE_CREATION or BEFORE_UPDATE) at which the Guard hook runs for the matched resources. (AI-inferred)
    invocation_points: Any = None
    # Specifies the list of AWS resource type names (for example, 'AWS::S3::Bucket') that this Guard hook evaluates for violations during CloudFormation operations. (AI-inferred)
    target_names: Any = None
    # Defines the resource targets that the Guard hook applies to, using include and exclude patterns based on resource type and resource identifiers. (AI-inferred)
    targets: Any = None

_GuardHook_RuleLocationFields = {
    "uri": ubx.FieldSpec(wire_name="uri"),
    "version_id": ubx.FieldSpec(wire_name="version_id"),
}

_GuardHook_StackFilters_StackNamesFields = {
    "exclude": ubx.FieldSpec(wire_name="exclude"),
    "include": ubx.FieldSpec(wire_name="include"),
}

_GuardHook_StackFilters_StackRolesFields = {
    "exclude": ubx.FieldSpec(wire_name="exclude"),
    "include": ubx.FieldSpec(wire_name="include"),
}

_GuardHook_StackFiltersFields = {
    "filtering_criteria": ubx.FieldSpec(wire_name="filtering_criteria"),
    "stack_names": ubx.FieldSpec(
        wire_name="stack_names",
        kind="object",
        fields=_GuardHook_StackFilters_StackNamesFields,
    ),
    "stack_roles": ubx.FieldSpec(
        wire_name="stack_roles",
        kind="object",
        fields=_GuardHook_StackFilters_StackRolesFields,
    ),
}

_GuardHook_TargetFilters_TargetsFields = {
    "action": ubx.FieldSpec(wire_name="action"),
    "invocation_point": ubx.FieldSpec(wire_name="invocation_point"),
    "target_name": ubx.FieldSpec(wire_name="target_name"),
}

_GuardHook_TargetFiltersFields = {
    "actions": ubx.FieldSpec(wire_name="actions"),
    "invocation_points": ubx.FieldSpec(wire_name="invocation_points"),
    "target_names": ubx.FieldSpec(wire_name="target_names"),
    "targets": ubx.FieldSpec(
        wire_name="targets",
        kind="list",
        fields=_GuardHook_TargetFilters_TargetsFields,
    ),
}

@dataclasses.dataclass
class GuardHookConfig:
    # The typename alias for the hook.
    alias: Any = None
    # IAM Role ARN
    execution_role: Any = None
    # Attribute to specify CloudFormation behavior on hook failure.
    failure_mode: Any = None
    # Attribute to specify which stacks this hook applies to or should get invoked for
    hook_status: Any = None
    # S3 Bucket where the guard validate report will be uploaded to
    log_bucket: Any = None
    # Specifies the Guard hook options, including the RuleStack property that gives the Amazon S3 location of the Guard rules file CloudFormation uses to apply policy validation. (AI-inferred)
    options: Any = None
    # S3 Source Location for the Guard files.
    rule_location: Any = None
    # Filters to allow hooks to target specific stack attributes
    stack_filters: Any = None
    # Attribute to specify which targets should invoke the hook
    target_filters: Any = None
    # Which operations should this Hook run against? Resource changes, stacks or change sets.
    target_operations: Any = None

@dataclasses.dataclass
class GuardHookAttrs:
    # The typename alias for the hook.
    alias: Any = None
    # IAM Role ARN
    execution_role: Any = None
    # Attribute to specify CloudFormation behavior on hook failure.
    failure_mode: Any = None
    # The Amazon Resource Name (ARN) of the activated hook
    hook_arn: Any = None
    # Attribute to specify which stacks this hook applies to or should get invoked for
    hook_status: Any = None
    # S3 Bucket where the guard validate report will be uploaded to
    log_bucket: Any = None
    # Specifies the Guard hook options, including the RuleStack property that gives the Amazon S3 location of the Guard rules file CloudFormation uses to apply policy validation. (AI-inferred)
    options: Any = None
    # S3 Source Location for the Guard files.
    rule_location: Any = None
    # Filters to allow hooks to target specific stack attributes
    stack_filters: Any = None
    # Attribute to specify which targets should invoke the hook
    target_filters: Any = None
    # Which operations should this Hook run against? Resource changes, stacks or change sets.
    target_operations: Any = None

GuardHook = ubx.ResourceBinding(
    wire_type="aws_cloud_formation_guard_hook",
    fields={
        "alias": ubx.FieldSpec(wire_name="alias"),
        "execution_role": ubx.FieldSpec(wire_name="execution_role"),
        "failure_mode": ubx.FieldSpec(wire_name="failure_mode"),
        "hook_status": ubx.FieldSpec(wire_name="hook_status"),
        "log_bucket": ubx.FieldSpec(wire_name="log_bucket"),
        "options": ubx.FieldSpec(wire_name="options"),
        "rule_location": ubx.FieldSpec(
            wire_name="rule_location",
            kind="object",
            fields=_GuardHook_RuleLocationFields,
        ),
        "stack_filters": ubx.FieldSpec(
            wire_name="stack_filters",
            kind="object",
            fields=_GuardHook_StackFiltersFields,
        ),
        "target_filters": ubx.FieldSpec(
            wire_name="target_filters",
            kind="object",
            fields=_GuardHook_TargetFiltersFields,
        ),
        "target_operations": ubx.FieldSpec(wire_name="target_operations"),
    },
)
