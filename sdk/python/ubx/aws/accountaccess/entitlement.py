# Code generated by `ubx sdk gen` from aws_account_access_entitlement. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class Entitlement_Entitlement_PrincipalRole_Principal_IdentityCenter:
    # The ID of the group
    group_id: Any = None
    # The ID of the user
    user_id: Any = None

@dataclasses.dataclass
class Entitlement_Entitlement_PrincipalRole_Principal:
    # Specifies the AWS IAM Identity Center (successor to AWS SSO) user or group that acts as the principal for the access entitlement, granting that identity access to the target account. (AI-inferred)
    identity_center: Any = None

@dataclasses.dataclass
class Entitlement_Entitlement_PrincipalRole:
    # The AWS account ID
    account: Any = None
    # The IAM user, role, or AWS account that is granted the account access entitlement and assumes the specified principal role in the target AWS account. (AI-inferred)
    principal: Any = None
    # The ARN of the IAM role
    role_arn: Any = None

@dataclasses.dataclass
class Entitlement_Entitlement:
    # Defines the IAM role principal, including its role name and optional organizational unit, that is granted the access entitlement to the specified target accounts. (AI-inferred)
    principal_role: Any = None

@dataclasses.dataclass
class EntitlementConfig:
    # The ARN of the application
    application_arn: Any = None

@dataclasses.dataclass
class EntitlementAttrs:
    # The ARN of the application
    application_arn: Any = None
    # The timestamp when the entitlement was created
    created_at: Any = None
    entitlement: Any = None
    # The ID of the entitlement
    entitlement_id: Any = None

Entitlement = ubx.ResourceBinding(
    wire_type="aws_account_access_entitlement",
    fields={
        "application_arn": ubx.FieldSpec(wire_name="application_arn"),
    },
)
