// Code generated by `ubx sdk gen` from aws_kafka_connect_connector. DO NOT EDIT.
package kafka

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type ConnectConnector_Capacity_AutoScaling_ScaleInPolicy struct {
	// Specifies the CPU utilization percentage threshold at which connector scale in should trigger.
	CpuUtilizationPercentage any
}

type ConnectConnector_Capacity_AutoScaling struct {
	// The maximum number of tasks allocated to the connector during autoscaling operations.
	MaxAutoscalingTaskCount any
	// The maximum number of workers for a connector.
	MaxWorkerCount any
	// Specifies how many MSK Connect Units (MCU) as the minimum scaling unit.
	McuCount any
	// The minimum number of workers for a connector.
	MinWorkerCount any
	// Information about the scale in policy of the connector.
	ScaleInPolicy any
	// Information about the scale out policy of the connector.
	ScaleOutPolicy any
}

type ConnectConnector_Capacity_ProvisionedCapacity struct {
	// Specifies how many MSK Connect Units (MCU) are allocated to the connector.
	McuCount any
	// Number of workers for a connector.
	WorkerCount any
}

type ConnectConnector_Capacity struct {
	// Details about auto scaling of a connector.
	AutoScaling any
	// Details about a fixed capacity allocated to a connector.
	ProvisionedCapacity any
}

type ConnectConnector_KafkaCluster_ApacheKafkaCluster_Vpc struct {
	// The AWS security groups to associate with the elastic network interfaces in order to specify what the connector has access to.
	SecurityGroups any
	// The list of subnets to connect to in the virtual private cloud (VPC). AWS creates elastic network interfaces inside these subnets.
	Subnets any
}

type ConnectConnector_KafkaCluster_ApacheKafkaCluster struct {
	// The bootstrap servers string of the Apache Kafka cluster.
	BootstrapServers any
	// Information about a VPC used with the connector.
	Vpc any
}

type ConnectConnector_KafkaCluster struct {
	// Details of how to connect to an Apache Kafka cluster.
	ApacheKafkaCluster any
}

type ConnectConnector_KafkaClusterClientAuthentication struct {
	// The type of client authentication used to connect to the Kafka cluster. Value NONE means that no client authentication is used.
	AuthenticationType any
}

type ConnectConnector_KafkaClusterEncryptionInTransit struct {
	// The type of encryption in transit to the Kafka cluster.
	EncryptionType any
}

type ConnectConnector_LogDelivery_WorkerLogDelivery_CloudWatchLogs struct {
	// Specifies whether the logs get sent to the specified CloudWatch Logs destination.
	Enabled any
	// The CloudWatch log group that is the destination for log delivery.
	LogGroup any
}

type ConnectConnector_LogDelivery_WorkerLogDelivery_Firehose struct {
	// The Kinesis Data Firehose delivery stream that is the destination for log delivery.
	DeliveryStream any
	// Specifies whether the logs get sent to the specified Kinesis Data Firehose delivery stream.
	Enabled any
}

type ConnectConnector_LogDelivery_WorkerLogDelivery_S3 struct {
	// The name of the S3 bucket that is the destination for log delivery.
	Bucket any
	// Specifies whether the logs get sent to the specified Amazon S3 destination.
	Enabled any
	// The S3 prefix that is the destination for log delivery.
	Prefix any
}

type ConnectConnector_LogDelivery_WorkerLogDelivery struct {
	// Details about delivering logs to Amazon CloudWatch Logs.
	CloudWatchLogs any
	// Details about delivering logs to Amazon Kinesis Data Firehose.
	Firehose any
	// Details about delivering logs to Amazon S3.
	S3 any
}

type ConnectConnector_LogDelivery struct {
	// Specifies where worker logs are delivered.
	WorkerLogDelivery any
}

type ConnectConnector_Plugins_CustomPlugin struct {
	CustomPluginArn any
	Revision any
}

type ConnectConnector_Plugins struct {
	CustomPlugin any
}

type ConnectConnector_Tags struct {
	Key any
	Value any
}

type ConnectConnector_WorkerConfiguration struct {
	// The revision of the worker configuration to use.
	Revision any
	// The Amazon Resource Name (ARN) of the worker configuration to use.
	WorkerConfigurationArn any
}

var ConnectConnector_Capacity_AutoScaling_ScaleInPolicyFields = ubx.FieldMap{
		"CpuUtilizationPercentage": ubx.FieldSpec{WireName: "cpu_utilization_percentage"},
	}

var ConnectConnector_Capacity_AutoScalingFields = ubx.FieldMap{
		"MaxAutoscalingTaskCount": ubx.FieldSpec{WireName: "max_autoscaling_task_count"},
		"MaxWorkerCount": ubx.FieldSpec{WireName: "max_worker_count"},
		"McuCount": ubx.FieldSpec{WireName: "mcu_count"},
		"MinWorkerCount": ubx.FieldSpec{WireName: "min_worker_count"},
		"ScaleInPolicy": ubx.FieldSpec{
			WireName: "scale_in_policy",
			Kind: "object",
			Fields: ConnectConnector_Capacity_AutoScaling_ScaleInPolicyFields,
		},
		"ScaleOutPolicy": ubx.FieldSpec{
			WireName: "scale_out_policy",
			Kind: "object",
			Fields: ConnectConnector_Capacity_AutoScaling_ScaleInPolicyFields,
		},
	}

var ConnectConnector_Capacity_ProvisionedCapacityFields = ubx.FieldMap{
		"McuCount": ubx.FieldSpec{WireName: "mcu_count"},
		"WorkerCount": ubx.FieldSpec{WireName: "worker_count"},
	}

var ConnectConnector_CapacityFields = ubx.FieldMap{
		"AutoScaling": ubx.FieldSpec{
			WireName: "auto_scaling",
			Kind: "object",
			Fields: ConnectConnector_Capacity_AutoScalingFields,
		},
		"ProvisionedCapacity": ubx.FieldSpec{
			WireName: "provisioned_capacity",
			Kind: "object",
			Fields: ConnectConnector_Capacity_ProvisionedCapacityFields,
		},
	}

var ConnectConnector_KafkaCluster_ApacheKafkaCluster_VpcFields = ubx.FieldMap{
		"SecurityGroups": ubx.FieldSpec{WireName: "security_groups"},
		"Subnets": ubx.FieldSpec{WireName: "subnets"},
	}

var ConnectConnector_KafkaCluster_ApacheKafkaClusterFields = ubx.FieldMap{
		"BootstrapServers": ubx.FieldSpec{WireName: "bootstrap_servers"},
		"Vpc": ubx.FieldSpec{
			WireName: "vpc",
			Kind: "object",
			Fields: ConnectConnector_KafkaCluster_ApacheKafkaCluster_VpcFields,
		},
	}

var ConnectConnector_KafkaClusterFields = ubx.FieldMap{
		"ApacheKafkaCluster": ubx.FieldSpec{
			WireName: "apache_kafka_cluster",
			Kind: "object",
			Fields: ConnectConnector_KafkaCluster_ApacheKafkaClusterFields,
		},
	}

var ConnectConnector_KafkaClusterClientAuthenticationFields = ubx.FieldMap{
		"AuthenticationType": ubx.FieldSpec{WireName: "authentication_type"},
	}

var ConnectConnector_KafkaClusterEncryptionInTransitFields = ubx.FieldMap{
		"EncryptionType": ubx.FieldSpec{WireName: "encryption_type"},
	}

var ConnectConnector_LogDelivery_WorkerLogDelivery_CloudWatchLogsFields = ubx.FieldMap{
		"Enabled": ubx.FieldSpec{WireName: "enabled"},
		"LogGroup": ubx.FieldSpec{WireName: "log_group"},
	}

var ConnectConnector_LogDelivery_WorkerLogDelivery_FirehoseFields = ubx.FieldMap{
		"DeliveryStream": ubx.FieldSpec{WireName: "delivery_stream"},
		"Enabled": ubx.FieldSpec{WireName: "enabled"},
	}

var ConnectConnector_LogDelivery_WorkerLogDelivery_S3Fields = ubx.FieldMap{
		"Bucket": ubx.FieldSpec{WireName: "bucket"},
		"Enabled": ubx.FieldSpec{WireName: "enabled"},
		"Prefix": ubx.FieldSpec{WireName: "prefix"},
	}

var ConnectConnector_LogDelivery_WorkerLogDeliveryFields = ubx.FieldMap{
		"CloudWatchLogs": ubx.FieldSpec{
			WireName: "cloud_watch_logs",
			Kind: "object",
			Fields: ConnectConnector_LogDelivery_WorkerLogDelivery_CloudWatchLogsFields,
		},
		"Firehose": ubx.FieldSpec{
			WireName: "firehose",
			Kind: "object",
			Fields: ConnectConnector_LogDelivery_WorkerLogDelivery_FirehoseFields,
		},
		"S3": ubx.FieldSpec{
			WireName: "s3",
			Kind: "object",
			Fields: ConnectConnector_LogDelivery_WorkerLogDelivery_S3Fields,
		},
	}

var ConnectConnector_LogDeliveryFields = ubx.FieldMap{
		"WorkerLogDelivery": ubx.FieldSpec{
			WireName: "worker_log_delivery",
			Kind: "object",
			Fields: ConnectConnector_LogDelivery_WorkerLogDeliveryFields,
		},
	}

var ConnectConnector_Plugins_CustomPluginFields = ubx.FieldMap{
		"CustomPluginArn": ubx.FieldSpec{WireName: "custom_plugin_arn"},
		"Revision": ubx.FieldSpec{WireName: "revision"},
	}

var ConnectConnector_PluginsFields = ubx.FieldMap{
		"CustomPlugin": ubx.FieldSpec{
			WireName: "custom_plugin",
			Kind: "object",
			Fields: ConnectConnector_Plugins_CustomPluginFields,
		},
	}

var ConnectConnector_TagsFields = ubx.FieldMap{
		"Key": ubx.FieldSpec{WireName: "key"},
		"Value": ubx.FieldSpec{WireName: "value"},
	}

var ConnectConnector_WorkerConfigurationFields = ubx.FieldMap{
		"Revision": ubx.FieldSpec{WireName: "revision"},
		"WorkerConfigurationArn": ubx.FieldSpec{WireName: "worker_configuration_arn"},
	}

type ConnectConnectorConfig struct {
	// Information about the capacity allocated to the connector.
	Capacity any
	// The configuration for the connector.
	ConnectorConfiguration any
	// A summary description of the connector.
	ConnectorDescription any
	// The name of the connector.
	ConnectorName any
	// Details of how to connect to the Kafka cluster.
	KafkaCluster any
	// Details of the client authentication used by the Kafka cluster.
	KafkaClusterClientAuthentication any
	// Details of encryption in transit to the Kafka cluster.
	KafkaClusterEncryptionInTransit any
	// The version of Kafka Connect. It has to be compatible with both the Kafka cluster's version and the plugins.
	KafkaConnectVersion any
	// Details of what logs are delivered and where they are delivered.
	LogDelivery any
	// The network type of the Connector.
	NetworkType any
	// List of plugins to use with the connector.
	Plugins any
	// The Amazon Resource Name (ARN) of the IAM role used by the connector to access Amazon S3 objects and other external resources.
	ServiceExecutionRoleArn any
	// A collection of tags associated with a resource
	Tags any
	// Specifies the worker configuration to use with the connector.
	WorkerConfiguration any
}

type ConnectConnectorAttrs struct {
	// Information about the capacity allocated to the connector.
	Capacity any
	// Amazon Resource Name for the created Connector.
	ConnectorArn any
	// The configuration for the connector.
	ConnectorConfiguration any
	// A summary description of the connector.
	ConnectorDescription any
	// The name of the connector.
	ConnectorName any
	// Details of how to connect to the Kafka cluster.
	KafkaCluster any
	// Details of the client authentication used by the Kafka cluster.
	KafkaClusterClientAuthentication any
	// Details of encryption in transit to the Kafka cluster.
	KafkaClusterEncryptionInTransit any
	// The version of Kafka Connect. It has to be compatible with both the Kafka cluster's version and the plugins.
	KafkaConnectVersion any
	// Details of what logs are delivered and where they are delivered.
	LogDelivery any
	// The network type of the Connector.
	NetworkType any
	// List of plugins to use with the connector.
	Plugins any
	// The Amazon Resource Name (ARN) of the IAM role used by the connector to access Amazon S3 objects and other external resources.
	ServiceExecutionRoleArn any
	// A collection of tags associated with a resource
	Tags any
	// Specifies the worker configuration to use with the connector.
	WorkerConfiguration any
}

var ConnectConnector = ubx.ResourceBinding{
	WireType: "aws_kafka_connect_connector",
	Fields: ubx.FieldMap{
		"Capacity": ubx.FieldSpec{
			WireName: "capacity",
			Kind: "object",
			Fields: ConnectConnector_CapacityFields,
		},
		"ConnectorConfiguration": ubx.FieldSpec{WireName: "connector_configuration"},
		"ConnectorDescription": ubx.FieldSpec{WireName: "connector_description"},
		"ConnectorName": ubx.FieldSpec{WireName: "connector_name"},
		"KafkaCluster": ubx.FieldSpec{
			WireName: "kafka_cluster",
			Kind: "object",
			Fields: ConnectConnector_KafkaClusterFields,
		},
		"KafkaClusterClientAuthentication": ubx.FieldSpec{
			WireName: "kafka_cluster_client_authentication",
			Kind: "object",
			Fields: ConnectConnector_KafkaClusterClientAuthenticationFields,
		},
		"KafkaClusterEncryptionInTransit": ubx.FieldSpec{
			WireName: "kafka_cluster_encryption_in_transit",
			Kind: "object",
			Fields: ConnectConnector_KafkaClusterEncryptionInTransitFields,
		},
		"KafkaConnectVersion": ubx.FieldSpec{WireName: "kafka_connect_version"},
		"LogDelivery": ubx.FieldSpec{
			WireName: "log_delivery",
			Kind: "object",
			Fields: ConnectConnector_LogDeliveryFields,
		},
		"NetworkType": ubx.FieldSpec{WireName: "network_type"},
		"Plugins": ubx.FieldSpec{
			WireName: "plugins",
			Kind: "list",
			Fields: ConnectConnector_PluginsFields,
		},
		"ServiceExecutionRoleArn": ubx.FieldSpec{WireName: "service_execution_role_arn"},
		"Tags": ubx.FieldSpec{
			WireName: "tags",
			Kind: "list",
			Fields: ConnectConnector_TagsFields,
		},
		"WorkerConfiguration": ubx.FieldSpec{
			WireName: "worker_configuration",
			Kind: "object",
			Fields: ConnectConnector_WorkerConfigurationFields,
		},
	},
}
