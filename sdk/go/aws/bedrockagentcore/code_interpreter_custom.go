// Code generated by `ubx sdk gen` from aws_bedrock_agent_core_code_interpreter_custom. DO NOT EDIT.
package bedrockagentcore

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type CodeInterpreterCustom_Certificates_CertificateLocation struct {
	// The ARN of the AWS Secrets Manager secret that stores the certificate for a custom certificate location, used by the AWS Bedrock Agent Core code interpreter custom resource to secure TLS connections. (AI-inferred)
	SecretArn any
}

type CodeInterpreterCustom_Certificates struct {
	// The certificate_location property of each certificate specifies the S3 URI of the certificate file that is mounted into the custom core code interpreter container to establish trust for secure connections. (AI-inferred)
	CertificateLocation any
}

type CodeInterpreterCustom_NetworkConfiguration_VpcConfig struct {
	// Security groups for VPC
	SecurityGroups any
	// Subnets for VPC
	Subnets any
}

type CodeInterpreterCustom_NetworkConfiguration struct {
	// Network modes supported by code interpreter
	NetworkMode any
	// Network mode configuration for VPC
	VpcConfig any
}

var CodeInterpreterCustom_Certificates_CertificateLocationFields = ubx.FieldMap{
		"SecretArn": ubx.FieldSpec{WireName: "secret_arn"},
	}

var CodeInterpreterCustom_CertificatesFields = ubx.FieldMap{
		"CertificateLocation": ubx.FieldSpec{
			WireName: "certificate_location",
			Kind: "object",
			Fields: CodeInterpreterCustom_Certificates_CertificateLocationFields,
		},
	}

var CodeInterpreterCustom_NetworkConfiguration_VpcConfigFields = ubx.FieldMap{
		"SecurityGroups": ubx.FieldSpec{WireName: "security_groups"},
		"Subnets": ubx.FieldSpec{WireName: "subnets"},
	}

var CodeInterpreterCustom_NetworkConfigurationFields = ubx.FieldMap{
		"NetworkMode": ubx.FieldSpec{WireName: "network_mode"},
		"VpcConfig": ubx.FieldSpec{
			WireName: "vpc_config",
			Kind: "object",
			Fields: CodeInterpreterCustom_NetworkConfiguration_VpcConfigFields,
		},
	}

type CodeInterpreterCustomConfig struct {
	// List of root CA certificates.
	Certificates any
	// Description of the code interpreter.
	Description any
	// The ARN of the IAM role.
	ExecutionRoleArn any
	// The name of the sandbox.
	Name any
	// Network configuration for code interpreter
	NetworkConfiguration any
	// A map of tag keys and values
	Tags any
}

type CodeInterpreterCustomAttrs struct {
	// List of root CA certificates.
	Certificates any
	// The ARN of a CodeInterpreter resource.
	CodeInterpreterArn any
	// The id of the code interpreter.
	CodeInterpreterId any
	// Timestamp when the code interpreter was created.
	CreatedAt any
	// Description of the code interpreter.
	Description any
	// The ARN of the IAM role.
	ExecutionRoleArn any
	// The reason for failure if the code interpreter creation or operation failed.
	FailureReason any
	// Timestamp when the code interpreter was last updated.
	LastUpdatedAt any
	// The name of the sandbox.
	Name any
	// Network configuration for code interpreter
	NetworkConfiguration any
	// Status of Code interpreter
	Status any
	// A map of tag keys and values
	Tags any
}

var CodeInterpreterCustom = ubx.ResourceBinding{
	WireType: "aws_bedrock_agent_core_code_interpreter_custom",
	Fields: ubx.FieldMap{
		"Certificates": ubx.FieldSpec{
			WireName: "certificates",
			Kind: "list",
			Fields: CodeInterpreterCustom_CertificatesFields,
		},
		"Description": ubx.FieldSpec{WireName: "description"},
		"ExecutionRoleArn": ubx.FieldSpec{WireName: "execution_role_arn"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"NetworkConfiguration": ubx.FieldSpec{
			WireName: "network_configuration",
			Kind: "object",
			Fields: CodeInterpreterCustom_NetworkConfigurationFields,
		},
		"Tags": ubx.FieldSpec{WireName: "tags"},
	},
}
