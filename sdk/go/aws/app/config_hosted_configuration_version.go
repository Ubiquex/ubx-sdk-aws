// Code generated by `ubx sdk gen` from aws_app_config_hosted_configuration_version. DO NOT EDIT.
package app

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type ConfigHostedConfigurationVersionConfig struct {
	// The application ID.
	ApplicationId any
	// The configuration profile ID.
	ConfigurationProfileId any
	// The content of the configuration or the configuration data.
	Content any
	// A standard MIME type describing the format of the configuration content.
	ContentType any
	// A description of the hosted configuration version.
	Description any
	// An optional locking token used to prevent race conditions from overwriting configuration updates when creating a new version. To ensure your data is not overwritten when creating multiple hosted configuration versions in rapid succession, specify the version number of the latest hosted configuration version.
	LatestVersionNumber any
	// A user-defined label for an AWS AppConfig hosted configuration version.
	VersionLabel any
}

type ConfigHostedConfigurationVersionAttrs struct {
	// The application ID.
	ApplicationId any
	// The configuration profile ID.
	ConfigurationProfileId any
	// The content of the configuration or the configuration data.
	Content any
	// A standard MIME type describing the format of the configuration content.
	ContentType any
	// A description of the hosted configuration version.
	Description any
	// An optional locking token used to prevent race conditions from overwriting configuration updates when creating a new version. To ensure your data is not overwritten when creating multiple hosted configuration versions in rapid succession, specify the version number of the latest hosted configuration version.
	LatestVersionNumber any
	// A user-defined label for an AWS AppConfig hosted configuration version.
	VersionLabel any
	// Current version number of hosted configuration version.
	VersionNumber any
}

var ConfigHostedConfigurationVersion = ubx.ResourceBinding{
	WireType: "aws_app_config_hosted_configuration_version",
	Fields: ubx.FieldMap{
		"ApplicationId": ubx.FieldSpec{WireName: "application_id"},
		"ConfigurationProfileId": ubx.FieldSpec{WireName: "configuration_profile_id"},
		"Content": ubx.FieldSpec{WireName: "content"},
		"ContentType": ubx.FieldSpec{WireName: "content_type"},
		"Description": ubx.FieldSpec{WireName: "description"},
		"LatestVersionNumber": ubx.FieldSpec{WireName: "latest_version_number"},
		"VersionLabel": ubx.FieldSpec{WireName: "version_label"},
	},
}
