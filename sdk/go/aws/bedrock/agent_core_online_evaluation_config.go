// Code generated by `ubx sdk gen` from aws_bedrock_agent_core_online_evaluation_config. DO NOT EDIT.
package bedrock

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type AgentCoreOnlineEvaluationConfig_ClusteringConfig struct {
	// The list of frequencies at which clustering reports are generated.
	Frequencies any
}

type AgentCoreOnlineEvaluationConfig_DataSourceConfig_CloudWatchLogs struct {
	// The list of CloudWatch log group names to monitor for agent traces.
	LogGroupNames any
	// The list of service names to filter traces within the specified log groups.
	ServiceNames any
}

type AgentCoreOnlineEvaluationConfig_DataSourceConfig struct {
	// The configuration for reading agent traces from CloudWatch logs.
	CloudWatchLogs any
}

type AgentCoreOnlineEvaluationConfig_Evaluators struct {
	EvaluatorId any
}

type AgentCoreOnlineEvaluationConfig_Insights struct {
	InsightId any
}

type AgentCoreOnlineEvaluationConfig_OutputConfig_CloudWatchConfig struct {
	// The CloudWatch log group name for evaluation results.
	LogGroupName any
}

type AgentCoreOnlineEvaluationConfig_OutputConfig struct {
	// The CloudWatch configuration for writing evaluation results.
	CloudWatchConfig any
}

type AgentCoreOnlineEvaluationConfig_Rule_Filters_Value struct {
	BooleanValue any
	DoubleValue any
	// Provides the string to match in a rule filter for the Bedrock Agent Core online evaluation configuration, when the filter's value type is STRING, to determine which interactions are evaluated. (AI-inferred)
	StringValue any
}

type AgentCoreOnlineEvaluationConfig_Rule_Filters struct {
	Key any
	Operator any
	Value any
}

type AgentCoreOnlineEvaluationConfig_Rule_SamplingConfig struct {
	// The percentage of agent traces to sample for evaluation.
	SamplingPercentage any
}

type AgentCoreOnlineEvaluationConfig_Rule_SessionConfig struct {
	// The number of minutes of inactivity after which an agent session is considered complete.
	SessionTimeoutMinutes any
}

type AgentCoreOnlineEvaluationConfig_Rule struct {
	// The list of filters that determine which agent traces should be included in the evaluation.
	Filters any
	// The configuration that controls what percentage of agent traces are sampled for evaluation.
	SamplingConfig any
	// The configuration that defines how agent sessions are detected.
	SessionConfig any
}

type AgentCoreOnlineEvaluationConfig_Tags struct {
	// The key of a tag attached to the Bedrock agent core online evaluation configuration, used to organize and identify the resource by custom metadata such as environment or project. (AI-inferred)
	Key any
	Value any
}

var AgentCoreOnlineEvaluationConfig_ClusteringConfigFields = ubx.FieldMap{
		"Frequencies": ubx.FieldSpec{WireName: "frequencies"},
	}

var AgentCoreOnlineEvaluationConfig_DataSourceConfig_CloudWatchLogsFields = ubx.FieldMap{
		"LogGroupNames": ubx.FieldSpec{WireName: "log_group_names"},
		"ServiceNames": ubx.FieldSpec{WireName: "service_names"},
	}

var AgentCoreOnlineEvaluationConfig_DataSourceConfigFields = ubx.FieldMap{
		"CloudWatchLogs": ubx.FieldSpec{
			WireName: "cloud_watch_logs",
			Kind: "object",
			Fields: AgentCoreOnlineEvaluationConfig_DataSourceConfig_CloudWatchLogsFields,
		},
	}

var AgentCoreOnlineEvaluationConfig_EvaluatorsFields = ubx.FieldMap{
		"EvaluatorId": ubx.FieldSpec{WireName: "evaluator_id"},
	}

var AgentCoreOnlineEvaluationConfig_InsightsFields = ubx.FieldMap{
		"InsightId": ubx.FieldSpec{WireName: "insight_id"},
	}

var AgentCoreOnlineEvaluationConfig_Rule_Filters_ValueFields = ubx.FieldMap{
		"BooleanValue": ubx.FieldSpec{WireName: "boolean_value"},
		"DoubleValue": ubx.FieldSpec{WireName: "double_value"},
		"StringValue": ubx.FieldSpec{WireName: "string_value"},
	}

var AgentCoreOnlineEvaluationConfig_Rule_FiltersFields = ubx.FieldMap{
		"Key": ubx.FieldSpec{WireName: "key"},
		"Operator": ubx.FieldSpec{WireName: "operator"},
		"Value": ubx.FieldSpec{
			WireName: "value",
			Kind: "object",
			Fields: AgentCoreOnlineEvaluationConfig_Rule_Filters_ValueFields,
		},
	}

var AgentCoreOnlineEvaluationConfig_Rule_SamplingConfigFields = ubx.FieldMap{
		"SamplingPercentage": ubx.FieldSpec{WireName: "sampling_percentage"},
	}

var AgentCoreOnlineEvaluationConfig_Rule_SessionConfigFields = ubx.FieldMap{
		"SessionTimeoutMinutes": ubx.FieldSpec{WireName: "session_timeout_minutes"},
	}

var AgentCoreOnlineEvaluationConfig_RuleFields = ubx.FieldMap{
		"Filters": ubx.FieldSpec{
			WireName: "filters",
			Kind: "list",
			Fields: AgentCoreOnlineEvaluationConfig_Rule_FiltersFields,
		},
		"SamplingConfig": ubx.FieldSpec{
			WireName: "sampling_config",
			Kind: "object",
			Fields: AgentCoreOnlineEvaluationConfig_Rule_SamplingConfigFields,
		},
		"SessionConfig": ubx.FieldSpec{
			WireName: "session_config",
			Kind: "object",
			Fields: AgentCoreOnlineEvaluationConfig_Rule_SessionConfigFields,
		},
	}

var AgentCoreOnlineEvaluationConfig_TagsFields = ubx.FieldMap{
		"Key": ubx.FieldSpec{WireName: "key"},
		"Value": ubx.FieldSpec{WireName: "value"},
	}

type AgentCoreOnlineEvaluationConfigConfig struct {
	// The configuration for clustering analysis of evaluation results.
	ClusteringConfig any
	// The configuration that specifies where to read agent traces for online evaluation.
	DataSourceConfig any
	// The description of the online evaluation configuration.
	Description any
	// The Amazon Resource Name (ARN) of the IAM role that grants permissions for evaluation.
	EvaluationExecutionRoleArn any
	// The list of evaluators to apply during online evaluation.
	Evaluators any
	ExecutionStatus any
	// The list of insights to enable for failure analysis.
	Insights any
	// The name of the online evaluation configuration. Must be unique within your account.
	OnlineEvaluationConfigName any
	// The evaluation rule that defines sampling configuration, filtering criteria, and session detection settings.
	Rule any
	// A list of tags to assign to the online evaluation configuration.
	Tags any
}

type AgentCoreOnlineEvaluationConfigAttrs struct {
	// The configuration for clustering analysis of evaluation results.
	ClusteringConfig any
	// The timestamp when the online evaluation configuration was created.
	CreatedAt any
	// The configuration that specifies where to read agent traces for online evaluation.
	DataSourceConfig any
	// The description of the online evaluation configuration.
	Description any
	// The Amazon Resource Name (ARN) of the IAM role that grants permissions for evaluation.
	EvaluationExecutionRoleArn any
	// The list of evaluators to apply during online evaluation.
	Evaluators any
	ExecutionStatus any
	// The list of insights to enable for failure analysis.
	Insights any
	// The Amazon Resource Name (ARN) of the online evaluation configuration.
	OnlineEvaluationConfigArn any
	// The unique identifier of the online evaluation configuration.
	OnlineEvaluationConfigId any
	// The name of the online evaluation configuration. Must be unique within your account.
	OnlineEvaluationConfigName any
	// The configuration that specifies where evaluation results should be written.
	OutputConfig any
	// The evaluation rule that defines sampling configuration, filtering criteria, and session detection settings.
	Rule any
	Status any
	// A list of tags to assign to the online evaluation configuration.
	Tags any
	// The timestamp when the online evaluation configuration was last updated.
	UpdatedAt any
}

var AgentCoreOnlineEvaluationConfig = ubx.ResourceBinding{
	WireType: "aws_bedrock_agent_core_online_evaluation_config",
	Fields: ubx.FieldMap{
		"ClusteringConfig": ubx.FieldSpec{
			WireName: "clustering_config",
			Kind: "object",
			Fields: AgentCoreOnlineEvaluationConfig_ClusteringConfigFields,
		},
		"DataSourceConfig": ubx.FieldSpec{
			WireName: "data_source_config",
			Kind: "object",
			Fields: AgentCoreOnlineEvaluationConfig_DataSourceConfigFields,
		},
		"Description": ubx.FieldSpec{WireName: "description"},
		"EvaluationExecutionRoleArn": ubx.FieldSpec{WireName: "evaluation_execution_role_arn"},
		"Evaluators": ubx.FieldSpec{
			WireName: "evaluators",
			Kind: "list",
			Fields: AgentCoreOnlineEvaluationConfig_EvaluatorsFields,
		},
		"ExecutionStatus": ubx.FieldSpec{WireName: "execution_status"},
		"Insights": ubx.FieldSpec{
			WireName: "insights",
			Kind: "list",
			Fields: AgentCoreOnlineEvaluationConfig_InsightsFields,
		},
		"OnlineEvaluationConfigName": ubx.FieldSpec{WireName: "online_evaluation_config_name"},
		"Rule": ubx.FieldSpec{
			WireName: "rule",
			Kind: "object",
			Fields: AgentCoreOnlineEvaluationConfig_RuleFields,
		},
		"Tags": ubx.FieldSpec{
			WireName: "tags",
			Kind: "list",
			Fields: AgentCoreOnlineEvaluationConfig_TagsFields,
		},
	},
}
